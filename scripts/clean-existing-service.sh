#!/usr/bin/env bash
# Clean stale Pact service processes before starting a new local service.
...
Options:
  ...
  --data-dir <path>     Data dir used by Pact service processes.
...
command_has_data_dir() {
  ...
  case "$command_line" in
    *"--data-dir $DATA_DIR"*|*"--data-dir=$DATA_DIR"*|*"PACT_SERVER_DATA_DIR=$DATA_DIR"*)
      return 0
      ;;
    *)
      return 1
...
kill_port_listeners() {
  ...
  if [[ "${#external_pids[@]}" -gt 0 ]]; then
    log "[clean] port ${port} is occupied by non-Pact process(es); refusing to stop them"
  ...
  if [[ "${#own_pids[@]}" -eq 0 ]]; then
    log "[clean] port ${port} has no Pact-owned listeners"
    return 0
  fi

  log "[clean] stopping Pact-owned listeners on port ${port}: ${own_pids[*]}"
  ...
  if [[ -n "$remaining" ]]; then
    ...
    if [[ "${#external_pids[@]}" -gt 0 ]]; then
      log "[clean] port ${port} is still occupied by non-Pact process(es); refusing to stop them"
      for pid in "${external_pids[@]}"; do
        describe_process "$pid"
      done
      return 1
    ...
    if [[ "${#own_pids[@]}" -gt 0 ]]; then
      log "[clean] force stopping Pact-owned listeners on port ${port}: ${own_pids[*]}"
      for pid in "${own_pids[@]}"; do
...
kill_stale_pact_processes() {
  ...
  pids="$(find_stale_pact_pids)"
  if [[ -z "$pids" ]]; then
    log "[clean] no stale Pact service processes for ${DATA_DIR}"
    return 0
  fi

  log "[clean] stopping stale Pact service processes: $(echo "$pids" | tr '\n' ' ')"
  for pid in $pids; do
  ...
  for i in $(seq 1 20); do
    ...
    if [[ "${#remaining[@]}" -eq 0 ]]; then
      log "[clean] stale Pact service processes stopped"
      return 0
    fi
    sleep 0.5
  done

  log "[clean] force stopping stale Pact service processes: ${remaining[*]}"
  for pid in "${remaining[@]}"; do
    kill -9 "$pid" 2>/dev/null || true
  done
}
...
log "[clean] existing Pact service cleanup complete"
