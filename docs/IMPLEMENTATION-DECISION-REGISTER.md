...
- 智能体兼容：不关心底层是什么大模型、agent framework 或机器人体系，统一通过 SplitAll MCP service / Workspace API 接入。
- 信息源兼容：不关心信息来自知识库、网站订阅、文件库、业务系统、人工整理或智能体上传文档，统一进入 workspace asset model。
- 工作空间环境兼容：不关心工作空间运行在容器、虚拟机、本机、云端、Linux、macOS 或 Windows；只要安装 SplitAll 管理软件，智能体访问工作空间必须经过 SplitAll。
...
| `DEC-P0-02` 接入面 | 智能体默认通过 SplitAll MCP service 接入；Workspace API 是协议事实源；其它入口只是适配。 |
...
| `DEC-P0-06` 上游再授权 | 所有上游对象进入 SplitAll 后必须生成 upstreamKnowledgeRef、derivedViewRef、derivedKnowledgeSpace、authorizationOverlay。 |
...
| `DEC-P0-10` 工作空间环境 | 受管工作空间必须安装 SplitAll 管理软件；智能体访问必须经过 adapter；直连文件系统视为未受管。 |
...
| `DEC-P1-05` durable execution | 语义先行：定义 `splitall.workflow.v1`，第一版自研轻量 runner 对齐 workflow/activity/retry/signal/timer/resume。 |
...
### DEC-P0-02 首选接入面是否确定为 SplitAll MCP service / Workspace API

需要决策：

- OpenClaw、Codex、Claude Code、Cursor Agent、脚本型 agent 是否统一通过 SplitAll MCP service / Workspace API 接入。
...
- 外部知识库进入 SplitAll 后是否必须生成 `upstreamKnowledgeRef`、`derivedViewRef`、`derivedKnowledgeSpace`、`authorizationOverlay`。
...
- SplitAll 管理软件在本机、容器、虚拟机、云端分别承担哪些职责。
- Linux、macOS、Windows 的路径、权限、文件监听、shell、进程能力如何抽象。
- 智能体是否只能通过管理软件访问受管工作空间，是否允许直连文件系统。

默认建议：受管工作空间必须经由 SplitAll adapter 访问；本地文件系统直连只能作为未受管区域，不进入公共 workspace state。

已决议：受管工作空间强制经过 SplitAll 管理软件和 adapter。
...
默认建议：先定义 `splitall.workflow.v1` 语义，第一版自研 runner 对齐接口，后续可替换。
...
- 是否支持多个 SplitAll 实例之间同步 workspace asset。
- 离线工作空间如何合并和冲突治理。
