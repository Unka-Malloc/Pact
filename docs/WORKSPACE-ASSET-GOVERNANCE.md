...
本文定义 SplitAll 的核心产品边界：公共工作空间资产治理。

SplitAll 不关心智能体之间如何互相协作，也不把智能体当作可信主体。系统只关心公共工作空间里的资产状态是否可治理。

## 定位

> SplitAll 是面向智能体时代的 Workspace Asset Governance System。
...
- SplitAll 的工作：把上游粗资源加工成可授权资产，把下游细贡献沉淀成可复用资产，并在中间统一快照、溯源、恢复、授权和排行。

三个兼容的边界如下：

- 智能体兼容：OpenClaw、Codex、Claude Code、Cursor Agent、其它机器人体系或脚本都不是核心抽象；统一通过 SplitAll MCP service / Workspace API 访问工作空间。
- 信息源兼容：知识库、网站订阅、文件库、业务系统、人工整理和智能体上传文档都先进入 workspace asset model，再统一治理。
- 工作空间环境兼容：容器、虚拟机、本机、云端、Linux、macOS、Windows 都只是环境差异；安装 SplitAll 管理软件后，智能体访问工作空间必须经过 SplitAll 的权限、路径、快照和审计适配。

本地智能体可以使用公共空间，但不能拥有公共空间。它们可以提交观察、产物、建议和轨迹；公共事实、公共资产和公共决策必须由 SplitAll 的状态机、策略和审计链确认。
...
SplitAll 用排行榜和统计面板管理贡献，而不是只依赖后台人工整理。
...
资产贡献统计报表是管理者视角的核心能力。因为 SplitAll 管的是公共空间，管理者必须能看到公共空间是否真的沉淀了团队资产，而不是只看到某个智能体完成了一次任务。
...
目标：证明两个本地 OpenClaw 可以通过 SplitAll 公共工作空间交换文档资产，而不需要两个 Agent 直接互调。

前提：

- 两个 OpenClaw 都安装访问本项目的 SplitAll MCP service。
...
2. SplitAll 创建上传记录、内容对象、资产快照、`contribution.submitted` 和 `auditId`，并把资产放入 `workspace/knowledge/` 或 `workspace/files/`。
...
5. B 请求下载时，SplitAll 校验 B 是否具备 `export` 或 `checkout` 权限；通过后生成 `knowledgeAccessReceipt`、`loanRecord`、`asset.downloaded` 和 `auditId`。
...
3. SplitAll 扫描 Skill 的权限需求、脚本风险、依赖和许可；通过后发布到 SkillLibrary、贡献面板和排行榜候选池。
4. B 在面板中看到该 Skill，也可以通过 SplitAll MCP service 调用 `workspace.skill.list` 发现它。
5. B 下载、安装或调用该 Skill 时，SplitAll 记录 `skill.downloaded`、`skill.installed` 或 `skill.used`，同时生成 `loanRecord` 和 `auditId`。
...
- Hermes：在 `~/.hermes/config.yaml` 增加 `mcp_servers.splitall`，指向 `http://host.orb.internal:8791/mcp`；修改前备份，只追加 splitall 条目。
...
- Demo workspace 可以清空重建；真实 agent 原配置只允许备份后追加 SplitAll 配置，不做破坏性重写。
- 手写 MCP 只覆盖本轮工具调用所需的最小协议，不追求完整 MCP server framework。

### 阶段一：MCP 服务 + 本机工作空间

目标：证明两个不同架构的本地智能体可以通过 SplitAll MCP service 操作同一个受管工作空间，不通过本地路径互相复制。
...
```text
title: SplitAll 简介
content: 可控的智能体协作空间。SplitAll 面向多个本地智能体提供公共工作空间、资产治理、权限控制、审计和可回档能力。
source: README.md
```
...
验收标准：A/B 都能通过 MCP 获取 `SplitAll 简介` evidence；每次访问都产生 `knowledgeAccessReceipt` 和 `auditId`；返回结果受流量限制。

### 阶段三：Skills 上传 + MCP 操作工作空间

目标：证明 SplitAll 不只交换文件和知识，也能让智能体贡献可复用能力，并通过 MCP 操作公共工作空间。
...
目标：在前三阶段链路跑通后关闭默认全开放权限，验证 SplitAll 的安全价值。
...
2. A 下载成功。
3. B 请求同一 asset id。
4. SplitAll 返回权限错误，包含 `auditId`，不返回文件内容。
...
- `P0-D Knowledge Seed + Search`：写入 `SplitAll 简介` seed knowledge，暴露 knowledge contribution/search/evidence，返回先做硬限制。
...
外部知识库是 workspace asset 的上游资产源，不是下游智能体的直接依赖。SplitAll 可以把外部知识库拿进来做信息切分和再授权，但智能体只能操作 SplitAll 派生出来的 workspace asset。

隔离要求：

- 下游智能体不能持有上游知识库 token。
- 下游智能体不能看到上游私有对象路径、collection id、索引 id 或裸 source id。
- 上游材料进入 SplitAll 后必须生成 `upstreamKnowledgeRef`，再映射到 SplitAll 自己的 asset/evidence id。
- SplitAll 必须能对同一份上游材料切出不同的 `derivedKnowledgeSpace`。
- 不同 subject / workspace / agent profile 读取的是不同 `authorizationOverlay`。
- 某些内容可以允许人看，但不允许智能体看。
- 某些内容可以允许智能体 readInPlace，但不允许 checkout/export。

这使 SplitAll 与外部知识库不会撞型：外部知识库负责保存或检索上游资产，SplitAll 负责把这些资产变成下游可治理、可快照、可审计、可恢复的工作空间资产视图。

因此，上游知识库的信息和资源权限再分配是 workspace asset governance 的核心能力，而不是外部连接器的附属功能。连接器只负责把上游资源带进来；SplitAll 必须负责重新切分、重新授权、重新登记、重新审计和在需要时撤销下游借阅。

## 演示场景：上游知识库 A/B 权限再授权

目标：证明上游知识库进入 SplitAll 后，不再按上游粗粒度权限直接暴露，而是由 SplitAll 在公共工作空间里做本地再授权。

流程：

1. SplitAll 从上游知识库获取某个文件，并为它建立 `upstreamKnowledgeRef`、SplitAll asset id、evidence id 和 `derivedKnowledgeSpace`。
...
5. 管理员点击“恢复到此节点”。
6. SplitAll 执行 `workspace.checkpoint.restore`，创建新的 restore operation，把当前 workspace 状态恢复为目标 checkpoint 的文件树、资产引用和权限视图。
...
实现上可以复用 git worktree 的成熟思想：文件 tree、diff、commit graph、checkout-like restore、临时 worktree 预览、branch 和 merge 都是合适的底层能力。边界是：SplitAll 的 workspace commit 还要覆盖数据库元数据、权限、知识 evidence、贡献和审计；所以 git 可以作为文件状态引擎或预览机制，但不能替代 Operation Ledger 和 Snapshot Boundary。
