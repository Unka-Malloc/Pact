...
SplitAll uses a prebuild gate so renderer builds cannot proceed until regression and smoke coverage pass. The smoke layer is intentionally smaller than the full release suite, but it must cover process startup, API reachability, CLI behavior, and memory growth in historically risky paths.
