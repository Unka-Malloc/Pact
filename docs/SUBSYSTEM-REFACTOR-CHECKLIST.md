...
| 18 | 补安全/审计/trace 生产硬化基础闭环 | 安全权限、接口封装层、工具管理、管理层 | `[x]` | 已新增 tenant/resource ABAC 裁决、console user tenant 属性、审计保留策略、脱敏审计导出和 trace drill-down API/RPC/CLI operation；`server:verify:security-hardening` 覆盖 tenant mismatch、workspace/dataClass/egress allowlist、denied request audit、脱敏 JSONL export、retention prune、真实启动后的 `/api/observability/traces/:traceId` 和 `/api/auth/audit/export`。 |
| 19 | 补控制台 session token rotation | 安全权限、接口封装层 | `[x]` | 已新增 `auth.sessions.rotate` API/RPC/CLI operation；当前 session 通过 CSRF 与 scope 校验后轮换 session token 和 HMAC CSRF token，旧 token 立即失效，并写入 console audit / operation audit；`server:verify:console-auth` 覆盖轮换前后登录态与旧 token 失效。 |
