...
| 2 | 输出 operation registry 差距清单 | 核心能力、接口封装层、应用层各能力 | `[x]` | 已落地 `reports/protocol-operation-registry-gap-2026-05-25.md`；当前 52 个协议操作全部注册，3 个仍是 `contract_registered` 空后端。 |
| 3 | 补齐 P0 协议缺口 | AgentLibrary、代码管理 | `[~]` | 共享空间协议已完成 operation/provider/verify 闭环；knowledge access 已有执行器；code review 已有 Gerrit upload route；剩余重点是 Codespace 持久化和 3 个知识/原始语料空后端。 |
