...
| 应用层 | 智能体 | `[~]` | 70% | agent context、agent memory、Agent Gateway、agent-configs 已从服务入口迁入 runtime provider 或 composition root；context preview/compaction/session memory 相关 handler 已拆分；settings 与 agent selector 的 console 投影已从 `api-facade` 下沉到 `console-state-projections`。 | handler context 仍有 runtime/store/service 直传；model routing、Agent Gateway 和 agent config 管理 handler 仍需进一步收敛到 agent provider/operation。 | context/memory/gateway/config 均只通过 agent provider/operation 暴露，接口层不直接操作 runtime。 |
