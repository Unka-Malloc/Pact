...
| 应用层 | 知识转化 | `[~]` | 65% | 架构图把知识蒸馏、知识索引、原始语料和 AgentLibrary 归为一个应用层模块；Knowledge console summary 已独立，`api-facade` 不再直接读 `runtime.mounts.knowledgeBase`；`knowledge-access` 已有 evaluate/receipt/loan/denied request 执行器。 | `raw-corpus.format.convert`、`knowledge.dossier.export`、`knowledge.distillation.export` 仍是 `contract_registered` 空后端；evidence/export/context injection 需要继续统一到源头权限裁决。 | 知识转化入口通过 operation/provider 调用；AgentLibrary 能完成 search/read evidence/export 并有 verify 覆盖。 |
...
| 应用层 | 共享空间 | `[~]` | 63% | 架构图将 `agent-workspace` 提升为应用层一等子系统，定位为受控文件空间、StateCommit、issue/proposal、锁和继承治理；workspace handler 已拆分；`workspace-contribution` 已有 registry-backed operation 实现；workspace file upload/list/download/read/write/patch 已闭合。 | checkpoint diff/restore/scope/revert 仍是 `contract_registered` 空后端；workspace runtime、issue/proposal、lock/inheritance 仍需继续协议化。 | workspace contribution 能完成注册、执行、审计和 verify；共享空间不再被视为单个智能体私有状态。 |
| 应用层 | 代码管理 / Codespace | `[~]` | 45% | 架构图新增 `pact.codespace` 和 `pact.code-review.v1` 应用能力；Gerrit upload route 已是真实后端；现有文档已明确代码贡献默认进入 Gerrit route。 | target/prepare/link/status 仍是轻量 protocol facade，缺 Codespace 持久化、review target registry、状态同步后端和验收脚本。 | 代码变更能通过 Codespace 形成 changeSet，并通过 Code Review/Gerrit route 完成 upload、review 状态同步、审计和 fallback。 |
...
| 2 | 输出 operation registry 差距清单 | 核心能力、接口封装层、应用层各能力 | `[x]` | 已落地 `reports/protocol-operation-registry-gap-2026-05-25.md`；当前 50 个协议操作全部注册，8 个仍是 `contract_registered` 空后端。 |
| 3 | 补齐 P0 协议缺口 | 共享空间、AgentLibrary、代码管理 | `[~]` | workspace contribution 与 knowledge access 已有执行器；code review 已有 Gerrit upload route；workspace file patch 已实现；剩余重点是 Codespace 持久化和 8 个空后端。 |
