...
| 应用层 | 共享空间 | `[~]` | 60% | 架构图将 `agent-workspace` 提升为应用层一等子系统，定位为受控文件空间、StateCommit、issue/proposal、锁和继承治理；workspace handler 已拆分；`workspace-contribution` 已有 registry-backed operation 实现。 | `workspace.file.patch` 与 checkpoint diff/restore/scope/revert 仍是 `contract_registered` 空后端；workspace file/runtime、issue/proposal、lock/inheritance 仍需继续协议化。 | workspace contribution 能完成注册、执行、审计和 verify；共享空间不再被视为单个智能体私有状态。 |
...
| 2 | 输出 operation registry 差距清单 | 核心能力、接口封装层、应用层各能力 | `[x]` | 已落地 `reports/protocol-operation-registry-gap-2026-05-25.md`；当前 50 个协议操作全部注册，9 个仍是 `contract_registered` 空后端。 |
| 3 | 补齐 P0 协议缺口 | 共享空间、AgentLibrary、代码管理 | `[~]` | workspace contribution 与 knowledge access 已有执行器；code review 已有 Gerrit upload route；剩余重点是 Codespace 持久化和 9 个空后端。 |
