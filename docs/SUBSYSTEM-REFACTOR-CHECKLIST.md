...
| 17 | 补 v0.0.1 外部凭据初始化入口 | 安全权限、代码管理、知识转化、共享空间、管理层 | `[x]` | 已新增本地 `pact secret <provider> init/list/status/targets` 和 `pact secret.<provider>.init` CLI；真实密钥仅写入 `ServerConfig.getDataDir()/secrets` 的 0600 本机 secret store，provider manifest 只保存 `secretRef` / `endpointRef` / `credentialConfigured`；`server:verify:secret-init` 覆盖 Gerrit、Dify 和 OneDrive 初始化、脱敏 registry、manifest 更新、audit 和 readiness 本地凭据识别。 |
