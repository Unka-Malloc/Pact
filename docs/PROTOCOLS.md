...
| `pact.client-runtime-bootstrap.v1` | 客户端主动声明平台、命令、模块需求和上传规模，服务端返回裁剪后的 Pact client runtime 模块计划与 transport 降级顺序。 |
...
| `pact.knowledge-access.v1` | source-level knowledge permissions、accessMode、checkoutPolicy、readInPlace、export 和 context injection 裁决。 |
...
1. 智能体兼容
   ...
   - OpenClaw、Codex、Claude Code、Cursor Agent、脚本型 agent 或人工 CLI 都通过 Pact MCP service / Workspace API 接入。
   - Pact 不暴露成必须被其它 agent 调度的自治 Agent；它暴露的是 workspace、asset、knowledge、checkpoint、permission 和 audit 能力。
2. 信息源兼容
   - 上游知识库、网站订阅、文件库、业务系统、人工整理、终端上传和智能体提交文档都统一进入 workspace asset model。
   - 智能体提交源代码、patch 或仓库变更时，兼容目标优先选择 Gerrit route；workspace asset model 只保存治理记录、引用和 fallback 资产。
   - 外部信息进入后必须被重新切分、标注、授权、索引、审计和快照，而不是按原系统权限裸转发。
3. 工作空间环境兼容
   - 容器、虚拟机、本机、云端、Linux、macOS、Windows 都只是 workspace runtime 的承载环境。
   - 只要安装 Pact 管理软件，智能体访问该工作空间就必须经过 Pact adapter，由 Pact 处理路径差异、权限控制、快照、恢复、审计和环境能力发现。
...
POST /api/workspaces/:workspaceId/code/targets/evaluate
POST /api/workspaces/:workspaceId/code/changes/prepare
POST /api/workspaces/:workspaceId/code/changes/upload
POST /api/workspaces/:workspaceId/code/changes/:codeChangeId/link
POST /api/workspaces/:workspaceId/code/changes/:codeChangeId/status/sync
...
`pact.backup-restore.v1` 管理服务端数据目录的备份、恢复预览和确认恢复。第一版恢复不删除备份中不存在的当前文件，只恢复 manifest 中声明的权威文件，避免误删运行期新增状态。
...
installer 只追加或替换 `pact` 这一项，必须先备份会被结构化写入的目标配置。不得覆盖、清空或重排用户已有 MCP server、API key、bot token 或 agent 配置。能用客户端标准 CLI 的目标必须调用标准 CLI；没有可脚本化标准 CLI 的目标由 `server:mcp:install` 按目标官方配置格式做结构化写入和备份。
...
`operation` 是 Pact 内部 Operation Registry / Tool Management 的操作 id。外部智能体不直接看到 100+ 个内部 operation；需要发现内部能力时，调用：

```text
pact.call({ "operation": "pact.capabilities.list" })
pact.call({ "operation": "pact.mcp.version" })
```
...
```json
{
  ...
  "params": {
    ...
    "stableToolName": "pact.call"
  }
}
```

不支持持续 SSE 的客户端通过下一次 `initialize`、`tools/list` 或 `pact.call({ "operation": "pact.mcp.version" })` 获取版本变化。只有 endpoint、auth 或客户端插件 manifest 变更时才需要重新运行 `pact-mcp register` 或按单客户端重新连接。
...
3. `tools/list` 是否只返回 `pact.call`。
4. `tools/call pact.call` 调用 `operation=system.health` 是否成功。
...
MCP handler 不能直接读写文件夹或知识库内部实现。所有 `tools/call pact.call` 必须落到现有 Operation Registry、Tool Management、Workspace API、Policy Engine、Operation Ledger、Checkpoint Tree 和 storage metadata。MCP adapter 只做协议转换、身份注入、版本协商、错误规范化和 streaming / stdio transport 兼容。
...
2. Pact 生成 `contribution.submitted`、`asset.created`、`snapshot.created` 和 `auditId`。
...
2. Pact 发布 Skill 到 `workspace/skills/`、SkillLibrary、贡献面板和 MCP skill list。
3. OpenClaw B 通过面板或 `workspace.skill.list` 看到该 Skill。
4. B 调用 `workspace.skill.download` 或安装后上报 `workspace.skill.usage.report`。
5. Pact 记录 `skill.downloaded`、`skill.installed` 或 `skill.used`，并执行 `usageCount += 1`；成功使用会提高 `successRate`，跨 workspace 采用会提高 `uniqueWorkspaceAdoptions`，随后刷新 `rankScoreV0`。
...
`pact.client-runtime-bootstrap.v1` 让本地客户端先声明环境，再由服务端返回可裁剪的 Pact client runtime 计划。它只规划框架、模块、依赖、签名校验和 transport 降级顺序；真实二进制 URL 由发布流水线或能力包生命周期填入。
...
- `installation`：安装根、签名校验要求、当前 artifact 状态和是否需要用户授权。
