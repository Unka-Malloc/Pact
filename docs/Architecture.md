...
- `server`
  - `Node.js` 服务端
  - 内部按 `modules / application / skills` 划分
...
- worker 队列与任务生命周期
- 下游 skill 管理
- `modules/FileProcessor` 文件处理模块加载、文档格式路由和归一化交付
- SQLite 元数据与检索索引
- 原始文件对象存储
...
PDF 不再直接归入通用 `documentParser`，默认先进入 `modules/FileProcessor/FileNormalizer/PDFProcessor`。`PDFProcessor` 内部可以调用 Tika，也可以在后续演进为先转 DOCX 再进入归一化流程。

### 3.4 Skills 与挂载

后端通过 `mount manager` 管理下游 skills。skills 是服务端可替换能力单元，挂载分两类：
...
文档归一化与文件处理统一收敛到 `server/modules/FileProcessor`。Node.js 主服务不直接解析文件，而是引入 FileProcessor 模块；FileProcessor 先按模块内路由表选择处理流程，再调用对应组件。
...
- `server/modules/FileProcessor/file-routing-table`
  - 文件类型到处理流程的注册表
...
- `server/modules/FileProcessor/FileNormalizer/Tika`
  - Tika 服务入口
  - 只读使用 Tika 运行时和 jar，不向 Tika 目录写运行文件
- `server/modules/FileProcessor/FileNormalizer/OCR`
  - OCR 服务入口
- `server/modules/FileProcessor/FileNormalizer/PDFProcessor`
  - PDF 专用流程入口，内部可调用 Tika
- `server/modules/FileProcessor/FileNormalizer/NormalizedDocuments`
...
- `server/modules/FileProcessor/module.json`
  - 管理 FileProcessor 自身组件清单、默认加载策略、可选打包组件和只读运行时约束

兼容策略：

- 启动时优先读取这两份新文件
- 如果新文件不存在，会兼容读取旧 `mounts.json`
- 一旦保存新配置，会写入两份新文件，并清理旧 `mounts.json`
...
4. 格式识别与路由
   - `modules/FileProcessor` 读取模块内 `file-routing-table`。
...
- 文件解析由 FileProcessor 内的路由表分派到 skills / 子组件，归一化 DOCX 由 `FileNormalizer/NormalizedDocuments` 承担。
...
- `server/application` 在创建 job 时把 `uploadSessionId` 转成 checkpoint receipt
...
- modules 提供稳定 API 和基础设施能力
- modules 不把具体业务流程写死在接口层
- modules 可以被 application 编排，也可以被 CLI / 运维命令直接调用
- modules 必须能声明组件、路由、依赖、引用关系和打包开关

### `server/application`

`application` 是服务端的业务编排层。

它负责：

- 作业流水线
- 上传完成后的任务创建、恢复、轮询和结果生成
- 文档格式路由后的处理流程编排
- 分析 skill 选择与调用
- 事务生命周期、线程归并、人物关系、lineage 和时间线恢复
- 搜索服务、删除协调器和结果归一化

原则：

- application 只编排业务流程，不直接实现外部能力
- application 通过 modules 读写状态、对象和配置
- application 通过 skills 执行可替换的解析、OCR、分析、知识库、向量库或图谱能力
- application 不直接持有 FileProcessor、VectorStore 等模块内部子组件
...
- skills 只暴露能力契约，不关心 HTTP / CLI / Flutter 如何触发
- skills 不拥有服务端全局状态；需要持久化时回到 modules 提供的存储接口
- 新 skill 应通过配置接入，不应要求改写 application 主流程
- 运行中的任务使用创建时的 skill 快照，新任务读取最新配置
...
通用知识库不再继续扩展这组应用层表。新的多模态知识库由 `server/modules/KnowledgeCore` 独立维护，应用层通过 `splitall.knowledge.v1` 调用：
...
- 文件型文档默认经 `modules/FileProcessor` 路由到 Tika、OCR、PDFProcessor 或自定义智能体
...
- `server/auth/ConsoleAuth`：本地用户、角色、会话、CSRF、OIDC 配置占位和审计日志。
- `server/interfaces/api/operation-registry.mjs`：所有 HTTP/RPC/CLI 接口声明 `requiredScopes`。
- `server/interfaces/http/controllers/system-controller.mjs`：只做请求解析、权限后处理和协议调用。
- `server/modules/KnowledgeCore`：继续作为 `knowledgeBase` mount，对外暴露 `splitall.knowledge.v1`。
