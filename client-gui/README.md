...
当前界面以 Stitch 设计稿 `Pact Console` 为参照，采用统一的控制台式信息架构：
...
```bash
pact-client server api GET /api/runtime/info
pact-client server api POST /api/knowledge/reindex '{"wait":true}' http://127.0.0.1:3000
```
...
客户端构建使用 `client-gui/packaging.modules.json` 决定打包哪些模块。默认包会包含 Flutter 前端、Rust CLI、Rust daemon、上传队列、断点续传、本地知识库镜像、智能体注册表、专家词汇、邮件索引和知识图谱 UI；macOS 还会在启用 `macos-mail-import` 时预编译 `pact-macos-mail-tool`。`macos-mail-import` 是 macOS-only 构建模块，Linux / Windows bundle 的运行时模块清单和产物目录都不会包含它。服务端模块资源默认不随客户端打包，需要时把对应 `server.*` 模块的 `enabled` 改为 `true`。
