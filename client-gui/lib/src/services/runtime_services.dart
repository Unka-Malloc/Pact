...
class ClientBackendApi {
  ...
  Future<bool> ensureDaemon() async {
    ...
    _spawnedDaemon ??= await Process.start(
      ...
      environment: {
        ...
        'PACT_PORTABLE_DIR': dataDirectory.path,
      },
  ...
  Future<ExportArtifact> exportResult({
    ...
    return ExportArtifact(
      fileName: (response['fileName'] ?? 'pact-result.${format.apiFormat}')
          .toString(),
  ...
  Future<Map<String, dynamic>> serverApi({
    ...
    final params = <String, dynamic>{
      'serviceBaseUrl': PactServiceUrls.normalizeBaseUrl(serviceBaseUrl),
      'method': method,
      'path': path,
    };
    if (body != null) {
  ...
  Future<BootstrapInfo> fetchBootstrap(String baseUrl) async {
    final decoded = await serverApi(
      serviceBaseUrl: PactServiceUrls.normalizeBaseUrl(baseUrl),
      method: 'GET',
      path: '/api/bootstrap',
    );
    ...
    final response = await _client
        .post(
          ...
          headers: {
            ...
            'x-pact-client-token': config.token,
          },
          body: jsonEncode({
            'jsonrpc': '2.0',
  ...
  Future<File?> _resolveDaemonBinary() async {
    ...
    final override = Platform.environment['PACT_CLIENTD_PATH'];
...
class PactServiceUrls {
  const PactServiceUrls._();
