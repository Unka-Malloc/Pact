...
final class MacOSMailImporter {
  ...
  private let queue = DispatchQueue(
    label: "pact.macos_mail_importer",
    qos: .userInitiated)
  private let stateQueue = DispatchQueue(
    label: "pact.macos_mail_importer.state")
#if canImport(FlutterMacOS)
...
private final class MailImportProgressMonitor {
  ...
  private let queue = DispatchQueue(
    label: "pact.macos_mail_importer.progress")
...
private final class MailExportDeduperCoordinator {
  ...
  private let queue = DispatchQueue(
    label: "pact.mail_export_deduper",
    qos: .utility)
  private var timer: DispatchSourceTimer?
  private var readOffset: UInt64 = 0
  private var pendingText = ""
...
private final class MailIndexCoordinator {
  ...
  private let queue = DispatchQueue(
    label: "pact.mail_index.coordinator",
    qos: .utility)
  private var timer: DispatchSourceTimer?
  private var readOffset: UInt64 = 0
  private var pendingText = ""
...
private final class MailIndexMergeScheduler {
  ...
  private let queue = DispatchQueue(
    label: "pact.mail_index.merge_scheduler",
    qos: .utility)
