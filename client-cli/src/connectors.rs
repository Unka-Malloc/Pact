...
fn oauth_manifest(provider_id: &str, scopes: &[&str]) -> Value {
    json!({
        "type": "oauth2",
        "providerId": provider_id,
        "authorizationUrl": format!("splitall://connectors/{}/oauth/start", provider_id),
...
fn invoke_connector_process(
    ...
    let request = json!({
        "protocolVersion": "splitall.data-connector.process.v1",
        "operation": operation,
        "providerId": provider_id,
        "params": params,
        "connector": manifest,
    ...
    let mut child = Command::new(&command_path)
        ...
        .env(
            "SPLITALL_CONNECTOR_PROTOCOL",
            "splitall.data-connector.process.v1",
        )
        .env("SPLITALL_CONNECTOR_OPERATION", operation)
        .env("SPLITALL_CONNECTOR_PROVIDER_ID", &provider_id)
        .env("SPLITALL_CONNECTOR_DATA_DIR", data_dir.as_os_str())
        .env("SPLITALL_CONNECTOR_MODULE_DIR", module_dir.as_os_str())
        .env("SPLITALL_CONNECTOR_CACHE_DIR", cache_dir.as_os_str())
        .env("SPLITALL_CONNECTOR_REMOTE_CALLS_ALLOWED", "0")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
