...
const DEFAULT_AGENT_GATEWAY = {
  alias:
    process.env.SPLITALL_AGENT_GATEWAY_ALIAS ||
    process.env.SPLITALL_CUSTOM_MODEL_ALIAS ||
    "external-agent",
  url: process.env.SPLITALL_AGENT_GATEWAY_URL || "",
  token: process.env.SPLITALL_AGENT_GATEWAY_TOKEN || "",
  tokenHeader: process.env.SPLITALL_AGENT_GATEWAY_TOKEN_HEADER || "token",
  tokenPrefix: process.env.SPLITALL_AGENT_GATEWAY_TOKEN_PREFIX || "",
  agentName: process.env.SPLITALL_AGENT_GATEWAY_AGENT_NAME || "",
  pluginList: [],
  engine: process.env.SPLITALL_AGENT_GATEWAY_ENGINE || "",
  parameters: {},
  timeoutMs: Number(process.env.SPLITALL_AGENT_GATEWAY_TIMEOUT_MS || 120000)
};
...
export const DEFAULT_SETTINGS = {
  ...
  customModelAlias:
    process.env.SPLITALL_CUSTOM_MODEL_ALIAS || DEFAULT_AGENT_GATEWAY.alias,
  ...
  customHttpAdapter: DEFAULT_AGENT_GATEWAY,
  customHttpAdapters: [],
  agentGateway: DEFAULT_AGENT_GATEWAY,
  analysisModuleId:
...
export function getAgentToolSettingsDirectory(userDataPath) {
  return path.join(userDataPath, "agent-tools");
}
...
function normalizeModelLibraryEntries(value, settings = {}) {
  ...
  if (incoming.length === 0 && !Object.hasOwn(settings || {}, "modelLibraryEntries")) {
    for (const model of Array.isArray(settings?.modelLibraryModels)
      ? settings.modelLibraryModels
      : []) {
      add(model?.provider);
    }
    const customHttpInput = {
      ...normalizePlainObject(settings?.agentGateway),
      ...normalizePlainObject(settings?.customHttpAdapter)
    };
    if (
...
function modelLibraryModelId(model = {}) {
...
function resolveModuleModelAssignmentToAgent(assignment, modelLibraryModels = []) {
  ...
  const providerModels = modelLibraryModels.filter(
  ...
  if (directMatch) {
    return {
      provider,
      model: modelLibraryModelId(directMatch)
    };
  }

  const legacyModelMatches = providerModels.filter((item) =>
  ...
  if (legacyModelMatches.length === 1) {
    return {
      provider,
      model: modelLibraryModelId(legacyModelMatches[0])
    };
  }
...
function normalizeModuleModelAssignments(
  ...
  const models = Array.isArray(modelLibraryModels) ? modelLibraryModels : [];
...
function normalizeAgentGateway(value = {}, fallbacks = {}) {
  const incoming = normalizePlainObject(value);
  const timeoutMs = Number(incoming.timeoutMs || DEFAULT_AGENT_GATEWAY.timeoutMs);
  const alias =
    String(
      incoming.alias ||
        incoming.modelAlias ||
        fallbacks.alias ||
        DEFAULT_AGENT_GATEWAY.alias ||
        ""
    ).trim() || DEFAULT_AGENT_GATEWAY.alias;
  return {
    alias,
    label:
      String(
    ...
    url: String(incoming.url || incoming.endpoint || DEFAULT_AGENT_GATEWAY.url || "").trim(),
    token: String(
      incoming.token ||
        incoming.apiKey ||
        fallbacks.token ||
        DEFAULT_AGENT_GATEWAY.token ||
        ""
    ).trim(),
    tokenHeader:
      String(incoming.tokenHeader || DEFAULT_AGENT_GATEWAY.tokenHeader || "token").trim() ||
      "token",
    tokenPrefix: String(incoming.tokenPrefix ?? DEFAULT_AGENT_GATEWAY.tokenPrefix ?? ""),
    agentName:
      readPresentString(incoming, ["agentName"]) ??
      String(DEFAULT_AGENT_GATEWAY.agentName || "").trim(),
    pluginList: normalizeStringList(incoming.pluginList || DEFAULT_AGENT_GATEWAY.pluginList),
    engine: String(incoming.engine || DEFAULT_AGENT_GATEWAY.engine || "").trim(),
    parameters: normalizePlainObject(incoming.parameters, {}),
    timeoutMs: Number.isFinite(timeoutMs) && timeoutMs > 0 ? timeoutMs : 120000
  };
}

function normalizeAgentGatewayList(primaryAdapter, settings = {}, fallbacks = {}) {
  const normalized = [];
  const seen = new Set();
  const addAdapter = (value, adapterFallbacks = {}) => {
    const adapter = normalizeAgentGateway(value, adapterFallbacks);
...
function normalizeModelLibraryModel(value = {}, index = 0, settings = {}) {
  const incoming = normalizePlainObject(value);
  ...
  const legacyInstanceId = String(incoming.instanceId || incoming.id || "").trim();
  const explicitUid = String(incoming.uid || "").trim();
  const legacyAlias = String(incoming.alias || incoming.modelAlias || "").trim();
  const uid =
    explicitUid ||
    (legacyInstanceId.startsWith("agent_") ? legacyInstanceId : "") ||
    (legacyAlias.startsWith("agent_") ? legacyAlias : "") ||
    stableAgentUid(provider, legacyInstanceId || legacyAlias || index + 1);
  const alias = uid;
  const timeoutMs = Number(
    incoming.timeoutMs ||
      (provider === "deepseek" ? settings.deepSeekTimeoutMs : undefined) ||
      DEFAULT_AGENT_GATEWAY.timeoutMs
  );
  return {
    uid,
    instanceId: uid,
    ...
    tokenHeader: String(incoming.tokenHeader || DEFAULT_AGENT_GATEWAY.tokenHeader || "token").trim() || "token",
    tokenPrefix: String(incoming.tokenPrefix ?? DEFAULT_AGENT_GATEWAY.tokenPrefix ?? ""),
    agentName,
    engine,
...
function normalizeModelLibraryModels(value = [], settings = {}) {
  const input = Array.isArray(value) ? value : [];
  const normalized = input
    .map((item, index) => normalizeModelLibraryModel(item, index, settings))
...
export function normalizeSettings(settings) {
  ...
  const customModelLabel =
    ...
    DEFAULT_SETTINGS.customModelLabel;
  const legacyAgentGateway = normalizePlainObject(settings?.agentGateway);
  const customHttpInput = {
    ...legacyAgentGateway,
    ...normalizePlainObject(settings?.customHttpAdapter)
  };
  ...
  const customHttpAdapter = normalizeAgentGateway(customHttpInput, {
    alias: customModelAlias,
    label: customModelLabel,
    token: settings?.customModelApiKey
  });
  const customHttpAdapters = normalizeAgentGatewayList(customHttpInput, settings, {
    alias: customModelAlias,
    label: customModelLabel,
    token: settings?.customModelApiKey
  });
  ...
  const modelLibraryModels = normalizeModelLibraryModels(settings?.modelLibraryModels, {
    ...settings,
    deepSeekModel,
  ...
  return {
    ...
    modelLibraryAgentIds: modelLibraryModels.map((model) => modelAgentId(model)).filter(Boolean),
    modelLibraryModels,
    ...
    customHttpAdapters,
    agentGateway: primaryCustomHttpAdapter,
...
function redactSettingsSecrets(settings) {
  ...
  return {
    ...
    modelLibraryModels: normalized.modelLibraryModels.map((item) => ({
      ...item,
      apiKey: "",
    ...
    agentGateway: {
      ...normalized.agentGateway,
      token: "",
      tokenConfigured: Boolean(normalized.agentGateway?.token)
    }
  };
}
...
function preserveModelLibraryModelSecrets(incomingModels, currentSettings) {
  if (!Array.isArray(incomingModels)) {
    return incomingModels;
  }
  const currentModels = normalizeSettings(currentSettings).modelLibraryModels || [];
...
async function writeSplitModelAgentSettings(userDataPath, settings = {}, options = {}) {
  ...
  const models = normalizeModelLibraryModels(settings.modelLibraryModels, settings);
...
async function saveSettingsUnlocked(userDataPath, incomingSettings, options = {}) {
  ...
  if (
    shouldPreserveProvider("custom-http") &&
    !String(incomingSettings?.agentGateway?.token || "").trim() &&
    current.agentGateway?.token
  ) {
  ...
  if (Array.isArray(incomingSettings?.modelLibraryModels)) {
    nextSettings.modelLibraryModels = preserveModelLibraryModelSecrets(
      incomingSettings.modelLibraryModels,
      current
    );
  }
  ...
  await atomicWriteJsonThroughState(settingsPath, stripProviderSettings({
    ...merged,
    modelLibraryAgentIds: merged.modelLibraryModels.map((model) => modelAgentId(model)).filter(Boolean)
  }), {
