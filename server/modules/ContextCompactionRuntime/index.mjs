...
function collectStructuredFacts(messages = [], runtimeState = {}) {
  ...
  return {
    ...
    knowledgePackages: [
      runtimeState.knowledgePackageVersion,
      runtimeState.knowledgePackageId,
      runtimeState.expertPackageVersion
...
function buildDeterministicSummary({
  ...
  const summary = [
    ...
    facts.knowledgePackages.length ? `Knowledge packages: ${facts.knowledgePackages.join(", ")}` : "",
    "Message notes:",
...
function buildModelPrompt({ messages, runtimeState, targetTokens, compactedRange }) {
  ...
  return [
    ...
    "Preserve user constraints, decisions, errors, TODOs, evidence/source ids, dates, amounts, file refs, tool call ids, and knowledge package versions.",
    "Return strict JSON with keys: summary, constraints, decisions, risks, todos, evidenceRefs, fileRefs, knowledgePackages.",
...
function buildReinjectionPayload({ input = {}, runtimeState = {}, policy }) {
  ...
  const candidates = [
    ...
    ["knowledgePackageVersion", source.knowledgePackageVersion || source.expertPackageVersion || input.knowledgePackageVersion || "", 65],
