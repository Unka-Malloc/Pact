...
function normalizeAgentGatewayEntry(value = {}, settings = {}, fallbacks = {}) {
  ...
  return {
    ...
    url: String(gateway.url || process.env.SPLITALL_AGENT_GATEWAY_URL || "").trim(),
    token: String(
      gateway.token ||
        gateway.apiKey ||
        fallbacks.token ||
        settings.customModelApiKey ||
        process.env.SPLITALL_AGENT_GATEWAY_TOKEN ||
        ""
    ).trim(),
    tokenHeader:
      String(
        gateway.tokenHeader || process.env.SPLITALL_AGENT_GATEWAY_TOKEN_HEADER || "token"
      ).trim() || "token",
    tokenPrefix: String(
      gateway.tokenPrefix ?? process.env.SPLITALL_AGENT_GATEWAY_TOKEN_PREFIX ?? ""
    ),
    agentName: String(
      gateway.agentName ||
        gateway.label ||
        process.env.SPLITALL_AGENT_GATEWAY_AGENT_NAME ||
        ""
    ).trim(),
    pluginList: asStringList(gateway.pluginList),
    engine: String(
      gateway.engine || gateway.model || process.env.SPLITALL_AGENT_GATEWAY_ENGINE || ""
    ).trim(),
    parameters: asPlainObject(gateway.parameters),
    timeoutMs: normalizeTimeout(
      gateway.timeoutMs || process.env.SPLITALL_AGENT_GATEWAY_TIMEOUT_MS
    ),
...
export function resolveAgentGatewayRegistry(settings = {}) {
  ...
  const addEntry = (entry, fallbacks = {}) => {
    ...
    const config = normalizeAgentGatewayEntry(entry, settings, fallbacks);
  ...
  addEntry(
    {
      ...asPlainObject(settings.agentGateway),
  ...
  for (const item of Array.isArray(settings.modelLibraryModels)
...
export function resolveAgentGatewayConfig(settings = {}, input = {}) {
  ...
  if (requestedAlias) {
    return (
      ...
      normalizeAgentGatewayEntry({ alias: requestedAlias }, settings)
    );
  }
  return registry[0] || normalizeAgentGatewayEntry({}, settings);
}
