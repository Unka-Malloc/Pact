...
function getVendorRoots() {
  const processCwdVendor = process.cwd() ? path.join(process.cwd(), "vendor") : "";
  const roots = [path.join(projectRoot, "vendor"), processCwdVendor];
...
async function resolveJavaCommand(settings = {}) {
  ...
  const vendorCandidates = getVendorRoots().flatMap((vendorRoot) => {
    const basePath = path.join(vendorRoot, "jre", getPlatformRuntimeFolderName());
    return [
