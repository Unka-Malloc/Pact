...
function getBundledOcrRuntimeRoots() {
  ...
  if (process.resourcesPath) {
    roots.push(path.join(process.resourcesPath, "vendor", "ocr-runtime", runtimeKey));
  }

  roots.push(path.join(process.cwd(), "vendor", "ocr-runtime", runtimeKey));
  roots.push(path.join(projectRoot, "vendor", "ocr-runtime", runtimeKey));
  return roots;
}
...
async function resolveOcrScriptPath() {
  const candidates = [path.join(projectRoot, "ocr", OCR_SCRIPT_NAME)];
  candidates.unshift(path.join(process.cwd(), "ocr", OCR_SCRIPT_NAME));

  if (process.resourcesPath) {
    candidates.unshift(path.join(process.resourcesPath, "ocr", OCR_SCRIPT_NAME));
  }
  ...
  throw createOcrError(
    "未找到 PaddleOCR 脚本。请确认 ocr/paddle_ocr_extract.py 已随程序分发。",
    "OCR_UNAVAILABLE"
  );
}
...
export async function extractTextWithPaddleOcr({
  ...
  try {
    ...
    if (error?.code === "ENOENT") {
      throw createOcrError(
        "未找到可用的 Python 运行环境。请填写 OCR Python 路径，或把运行时放到 vendor/ocr-runtime/<platform-arch>/。",
        "OCR_UNAVAILABLE"
      );
    }
