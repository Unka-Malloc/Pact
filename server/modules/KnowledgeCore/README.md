...
Optional LlamaIndex + LanceDB support is exposed through JavaScript adapters or external services behind `server/modules/LearningRuntime` and the LanceDB vector adapter. Operators must install and configure those external components explicitly; startup must not download model weights implicitly.
