...
import { createEmbeddingRuntime as createProtocolEmbeddingRuntime } from "../EmbeddingRuntime/index.mjs";
import {
  SQLITE_VEC_PROVIDER_ID,
  createLocalVectorStore as createProtocolLocalVectorStore
} from "../VectorStore/LocalVectorStore/index.mjs";
import {
  LEARNING_PROTOCOL_VERSION,
  createLearningRuntime
} from "../LearningRuntime/index.mjs";
import { createKnowledgeTaxonomyRuntime } from "../../knowledge-taxonomy.mjs";
import {
...
function resolveQueryIntentProfile(query, taxonomy = null) {
  return resolveTaxonomyQueryIntentProfile(query, taxonomy);
}

function queryTermsForSearch(query, intentProfile = null) {
  return queryTermsForIntentSearch(queryTerms(query), intentProfile, 80);
}

function tokenOverlapScore(text, queryTokens) {
...
function termPresenceScore(text, terms) {
  if (!terms.length) {
    return 1;
  }
...
function candidateTemporalSource(row = {}) {
...
function exponentialRecencyScore(timestampMs, referenceMs, retrievalSettings = {}) {
...
function hydrateRetrievalProfile(row) {
  if (!row) return null;
  return {
...
function hydrateLearningRun(row) {
  if (!row) return null;
  return {
    runId: row.run_id,
    status: row.status,
...
function hydrateProfileDeployment(row) {
  if (!row) return null;
  return {
...
function createKnowledgeStore({ db, rootPath, taxonomyRuntime = null }) {
  ensureSchema(db);
  ...
  function rebuildHierarchyIndex() {
    ...
    for (const collection of collections) {
      ...
      upsertHierarchyNode({
        ...
        summary: collection.collection_type,
        text: documents.map((document) => `${document.title}\n${document.summary}`).join("\n"),
        categoryPath: [collection.collection_type, collection.title],
    ...
    for (const document of documents) {
      const sections = db.prepare("SELECT * FROM kc_sections WHERE document_id = ? ORDER BY position ASC").all(document.document_id);
      const blocks = db.prepare("SELECT * FROM kc_blocks WHERE document_id = ? ORDER BY position ASC LIMIT 24").all(document.document_id);
      const metadata = parseJson(document.metadata_json, {});
      upsertHierarchyNode({
        nodeType: "document",
        targetId: document.document_id,
        parentHierarchyId: hierarchyId("collection", document.collection_id),
        collectionId: document.collection_id,
        documentId: document.document_id,
        batchId: document.batch_id,
        title: document.title,
        summary: document.summary,
        text: boundedIndexText([
          document.summary,
          sections.map((section) => section.title).join("\n"),
          blocks.map((block) => `${block.title}\n${block.snippet || block.text}`).join("\n")
        ]),
        categoryPath: [metadata.source, document.document_type, metadata.categories, document.title],
        metadata
      });
      for (const section of sections) {
        const sectionBlocks = blocks.filter((block) => block.section_id === section.section_id);
        const sectionMetadata = parseJson(section.metadata_json, {});
        upsertHierarchyNode({
          nodeType: "section",
          targetId: section.section_id,
          parentHierarchyId: hierarchyId("document", document.document_id),
          collectionId: document.collection_id,
          documentId: document.document_id,
          ...
          text: boundedIndexText(sectionBlocks.map((block) => `${block.title}\n${block.snippet || block.text}`)),
          categoryPath: [document.document_type, document.title, section.title],
  ...
  function upsertKnowledgeDocument(document) {
    ...
    const documentBlocks = asArray(document.blocks);
    const documentSections = asArray(document.sections);
    const documentAssets = asArray(document.assets);
    const documentText = boundedIndexText([
      document.summary || "",
      documentSections.map((section) => section.title || "").join("\n"),
      documentBlocks.map((block) => `${block.title || ""}\n${searchableBlockSnippet(block) || searchableBlockText(block)}`).join("\n"),
      documentAssets.map((asset) => `${asset.title || ""}\n${asset.caption || asset.ocrText || asset.text || ""}`).join("\n")
    ]);
    upsertHierarchyNode({
      nodeType: "collection",
      targetId: document.collectionId,
      collectionId: document.collectionId,
      batchId: document.batchId || "",
      title: document.collectionTitle || document.collectionId,
      summary: document.collectionType || "collection",
      text: `${document.title || ""}\n${document.summary || ""}`,
      categoryPath: [document.collectionType || "collection", document.collectionTitle || document.collectionId],
    ...
    for (const section of documentSections) {
      const sectionBlocks = documentBlocks.filter((block) => (block.sectionId || "") === section.sectionId);
      const sectionAssets = documentAssets.filter((asset) => (asset.sectionId || "") === section.sectionId);
      upsertHierarchyNode({
        nodeType: "section",
        targetId: section.sectionId,
        parentHierarchyId: hierarchyId("document", document.documentId),
        collectionId: document.collectionId,
        documentId: document.documentId,
        sectionId: section.sectionId,
        batchId: document.batchId || "",
        ...
        summary: document.title || "",
        text: boundedIndexText([
          sectionBlocks.map((block) => `${block.title || ""}\n${searchableBlockSnippet(block) || searchableBlockText(block)}`).join("\n"),
          sectionAssets.map((asset) => `${asset.title || ""}\n${asset.caption || asset.ocrText || asset.text || ""}`).join("\n")
        ]),
        categoryPath: [
          document.documentType || "",
          document.title || "",
  ...
  function lexicalBlockCandidates({ query, queryTokens, batchId = "", limit = 120 }) {
    const filters = [];
    const params = [];
    if (batchId) {
    ...
    return db.prepare(`
      ...
      LIMIT ?
    `).all(normalizeText(query), like, like, like, ...params, limit).map((row) => ({
      targetType: "block",
      row,
      score: queryTokens.length ? tokenOverlapScore(`${row.title}\n${searchableText(row.text, 6000)}`, queryTokens) : 0.6,
      reason: "like"
    }));
  }

  function lexicalAssetCandidates({ query, queryTokens, batchId = "", limit = 120 }) {
    const filters = [];
    const params = [];
    if (batchId) {
      filters.push("d.batch_id = ?");
    ...
    return db.prepare(`
      ...
      LIMIT ?
    `).all(normalizeText(query), like, like, like, like, ...params, limit).map((row) => ({
      targetType: "asset",
      row,
  ...
  function vectorCandidates({ query, queryTokens = [], batchId = "", limit = 120, settings = DEFAULT_SETTINGS }) {
    const blockById = new Map(
      db.prepare(`
        ...
        JOIN kc_documents d ON d.document_id = b.document_id
        WHERE (? = '' OR d.batch_id = ?)
      `).all(batchId, batchId).map((row) => [row.block_id, row])
    );
    const assetById = new Map(
      db.prepare(`
        ...
        JOIN kc_documents d ON d.document_id = a.document_id
        WHERE (? = '' OR d.batch_id = ?)
      `).all(batchId, batchId).map((row) => [row.asset_id, row])
    );
  ...
  function hierarchyNodeCandidates({ query, queryTokens, batchId = "", limit = 40 }) {
  ...
  function buildHierarchyPlan({ query, queryTokens, batchId = "", settings, safeLimit }) {
    ...
    const nodes = hierarchyNodeCandidates({
      query,
    ...
    const selectedNodes = nodes.filter((node) => node.score >= threshold).slice(0, branchTopK);
  ...
  function hierarchyRouteForCandidate(candidate, plan) {
    ...
    const score = Math.max(documentScore, sectionScore);
    if (score <= 0) {
      return null;
    }
    return {
  ...
  function hierarchyBackoffCandidates({ plan, limit = 16 }) {
    ...
    const documentIds = [...plan.selectedDocumentIds].slice(0, safeLimit);
    const candidates = [];
  ...
  function search(input = {}) {
    ...
    for (const candidate of lexicalBlockCandidates({ query, queryTokens, batchId, limit: candidateLimit })) {
      generatedCandidates.push(candidate);
    }
    for (const candidate of lexicalAssetCandidates({ query, queryTokens, batchId, limit: candidateLimit })) {
      generatedCandidates.push(candidate);
    }
    if (query && !keywordOnly) {
      for (const candidate of vectorCandidates({ query, queryTokens, batchId, limit: candidateLimit, settings })) {
        generatedCandidates.push(candidate);
      }
    }
  ...
  function runGarbageCleanup(input = {}) {
    ...
    return {
      ...
      notes: [
        ...
        "裁剪 kc_sync_log 后，早于 syncLogCutoffCursor 的客户端需要做一次完整同步。",
...
export function createKnowledgeCoreMount({ userDataPath }) {
  ...
  const store = createKnowledgeStore({ db, rootPath, taxonomyRuntime });
