...
function compactEvidenceItem(item = {}, index = 0) {
  return {
    ...
    itemId: String(item.itemId || item.documentId || ""),
    documentId: String(item.documentId || item.itemId || ""),
    ...
    sourceLocator: item.sourceLocator || item.source || null,
  ...
  function evidenceItemsFromOpened(openedEvidence = []) {
    return asArray(openedEvidence)
      .map((evidence, index) => ({
        ...
        itemId: normalizeText(evidence.documentId),
        documentId: normalizeText(evidence.documentId || evidence.evidenceId),
        title: normalizeText(evidence.title || evidence.evidenceId),
        snippet: truncateText(evidence.snippet || evidence.text || "", 900),
        score: 1,
        hierarchy: null,
        sourceLocator: evidence.sourcePath ? { path: evidence.sourcePath } : null,
