...
import { isSupportedImportFilePath } from "../modules/FileProcessor/index.mjs";
...
import { atomicWriteJson } from "./state-coordinator.mjs";
...
function normalizeSource(input = {}, previous = {}) {
  ...
  const sourceId = String(previous.sourceId || input.sourceId || `ks_${randomUUID()}`).trim();
  ...
  function sourceWatchSignature(source) {
    ...
    try {
      ...
      await publish(sourceId, "knowledge.sources.watch_error");
	    ...
	    const activeSyncRun = activeSyncRuns.get(sourceId);
	    if (activeSyncRun) {
	      const activeJob = source.lastJobId ? await jobManager.getJob(source.lastJobId) : null;
	      return {
	        skipped: true,
	        reason: "sync_active",
	        source: publicSource(source, activeJob)
	      };
	    }
	    const run = Promise.resolve().then(async () => {
	    ...
	    await startCheckpointTree({
	      ...
	      resumePolicy: {
	        ...
	        idempotencyKey: "sourceId+sourceManifestSha256+hydratedManifestSha256",
	        reusableState: "knowledge-sources/hydrated + jobs payload/checkpoint receipt"
	      },
	    ...
	    if (activeJob && ["queued", "running"].includes(activeJob.status)) {
	      ...
	      await finishCheckpointTree({
	        ...
	        treeId: syncTreeId,
	        status: "skipped",
	        message: "Source sync reused active parse job.",
	        metadata: {
        ...
        lastJobStatus: activeJob.status,
        lastJobStage: activeJob.stage,
        lastJobProgressPercent: Number(activeJob.progressPercent || 0),
        updatedAt: nowIso()
      });
      await persist();
      await publish(sourceId, "knowledge.sources.sync_waiting");
      return {
        skipped: true,
        reason: "job_active",
        source: publicSource(source, activeJob)
      ...
      updatedAt: nowIso()
	    ...
	    if (scanned.fileCount === 0) {
	      ...
	      await finishCheckpointTree({
	        ...
	        treeId: syncTreeId,
	        status: "skipped",
	        message: "Source directory has no parseable files."
	      });
	      Object.assign(source, {
        status: "idle",
        error: "目录中没有可解析的文件。",
      ...
      await publish(sourceId, "knowledge.sources.scan_empty");
      return {
        skipped: true,
        reason: "empty",
        source: publicSource(source)
	    ...
	    if (
	      !force &&
	      source.lastSnapshotHash === scanned.manifestSha256 &&
	      Number(source.lastHydrationFailedCount || 0) === 0
	    ) {
	      await upsertCheckpointNode({
	        userDataPath,
	        treeId: syncTreeId,
	      ...
	      await finishCheckpointTree({
	        ...
	        treeId: syncTreeId,
	        status: "completed",
	        message: "Source sync reused unchanged checkpoint.",
	        metadata: {
	          sourceManifestSha256: scanned.manifestSha256
	        }
	      });
    ...
    await publish(sourceId, "knowledge.sources.sync_started");
	    return {
	      ...
	      const message = error instanceof Error ? error.message : "目录同步失败。";
	      await upsertCheckpointNode({
	        userDataPath,
	        treeId: syncTreeId,
	        nodeId: "source-sync-error",
	        parentId: "source-sync",
	        label: "目录同步失败",
	        status: "failed",
	        error: message
	    ...
	    activeSyncRuns.set(sourceId, run);
	    run.finally(() => {
	      if (activeSyncRuns.get(sourceId) === run) {
    ...
    timers.set(
      sourceId,
	      setTimeout(() => {
	        ...
	        void triggerSourceIndex(sourceId, { reason: "watch" }).catch(async (error) => {
	          const current = sources.get(sourceId);
	          if (!current) {
	            return;
	          }
	          Object.assign(current, {
	            indexStatus: "failed",
	            lastIndexError: error instanceof Error ? error.message : "原文倒排索引失败。",
	            updatedAt: nowIso()
	          });
	          await persist();
	          await publish(sourceId, "knowledge.sources.index_failed");
  ...
  return {
	    async start() {
	      ...
	      for (const source of sources.values()) {
	        if (source.enabled && !source.lastIndexSnapshotHash) {
	          if (sourceStartupIndexEnabled()) {
	            void triggerSourceIndex(source.sourceId, {
	              reason: "startup_missing_index",
	              force: false
	            }).catch(() => null);
	            continue;
	          }
	          Object.assign(source, {
	            indexStatus: "pending",
	            lastIndexReason: "startup_missing_index_deferred",
	            pendingReason: "source_index_deferred_until_manual_refresh",
	            updatedAt: nowIso()
	          });
	        }
	      }
    ...
    async createSource(input = {}) {
      ...
      if (existing) {
        const updated = normalizeSource({ ...input, directoryPath: existing.directoryPath }, existing);
	        ...
	        const indexRun = triggerSourceIndex(existing.sourceId, {
	          reason: "source_registered",
	          force: input.forceIndex === true
	        }).catch(() => null);
	      ...
	      const indexRun = triggerSourceIndex(next.sourceId, {
	        reason: "source_registered",
	        force: true
	      }).catch(() => null);
	      const result = input.runNow === false
    ...
    async updateSource(sourceId, patch = {}) {
      ...
      const next = normalizeSource({ ...current, ...patch, sourceId }, current);
	      ...
	      await startWatchingSource(sourceId);
	      if (next.enabled) {
	        void triggerSourceIndex(sourceId, {
    ...
    async deleteSource(sourceId) {
      ...
      return {
	    ...
	    async refreshSource(sourceId, options = {}) {
	      await refreshInMemorySources();
	      const indexRun = triggerSourceIndex(sourceId, {
