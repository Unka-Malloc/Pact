...
import { removeImportCheckpoint } from "../application/import-resume-store.mjs";
...
import { isServerToken, serverToken } from "../security/client-strings.mjs";
...
export function createJobManager({
  ...
  function rememberActiveManifestJob(job) {
    ...
    const existingJobId = activeManifestJobs.get(manifestKey);
    ...
    if (!existingJob || String(job.createdAt || "") < String(existingJob.createdAt || "")) {
      activeManifestJobs.set(manifestKey, job.id);
    }
  }

  function forgetActiveManifestJob(job) {
    const manifestKey = normalizeManifestKey(job);
    if (manifestKey && activeManifestJobs.get(manifestKey) === job?.id) {
      activeManifestJobs.delete(manifestKey);
    }
  }

  function getActiveManifestJob(manifestKey) {
    if (!manifestKey) {
      return null;
    }
    const existingJobId = activeManifestJobs.get(manifestKey);
  ...
  return {
    async createJob(payload) {
      ...
      const existingManifestJob = getActiveManifestJob(manifestKey);
