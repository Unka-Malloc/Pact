...
const userDataPath = await fs.mkdtemp(path.join(os.tmpdir(), "splitall-knowledge-evolution-loop-"));
...
try {
  ...
  assert.equal(roles.protocolVersion, "splitall.model-decision.v1");
  ...
  assert.equal(evolution.protocolVersion, "splitall.knowledge-evolution.v1");
  ...
  assert.equal(skillRun.protocolVersion, "splitall.knowledge-agent-skill.v1");
  ...
  assert.equal(hierarchyAudit.protocolVersion, "splitall.knowledge-evolution.v1");
  ...
  assert.equal(evolutionRun.protocolVersion, "splitall.knowledge-evolution.v1");
