...
async function startMockQdrant() {
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      if (request.method === "GET" && url.pathname === "/collections/splitall_external_test") {
      ...
      if (request.method === "PUT" && url.pathname === "/collections/splitall_external_test") {
        state.collectionCreated = true;
        await readJson(request);
        response.end(JSON.stringify({ result: true }));
        return;
      }
      if (request.method === "PUT" && url.pathname === "/collections/splitall_external_test/points") {
      ...
      if (request.method === "POST" && url.pathname === "/collections/splitall_external_test/points/search") {
        const body = await readJson(request);
      ...
      if (request.method === "POST" && url.pathname === "/collections/splitall_external_test/points/delete") {
        const body = await readJson(request);
...
async function startMockOpenSearch() {
  ...
  const server = http.createServer(async (request, response) => {
    ...
    response.setHeader("content-type", "application/json");
    try {
      if (request.method === "HEAD" && url.pathname === "/splitall_external_test") {
        response.statusCode = state.indexCreated ? 200 : 404;
        response.end();
        return;
      }
      if (request.method === "PUT" && url.pathname === "/splitall_external_test") {
      ...
      if (request.method === "POST" && url.pathname === "/splitall_external_test/_search") {
        const body = await readJson(request);
      ...
      if (request.method === "POST" && url.pathname === "/splitall_external_test/_delete_by_query") {
        const body = await readJson(request);
...
async function assertQdrantAdapter() {
  ...
  const userDataPath = await fs.mkdtemp(path.join(os.tmpdir(), "splitall-external-kb-"));
  const mount = await createExternalKnowledgeBaseMount({
    ...
    runtimeOptions: {
      externalKnowledgeBase: {
        ...
        collection: "splitall_external_test",
        dimension: 128
      }
    }
  });
  ...
  try {
    ...
    assert.match(evidence.markdown, /splitall_knowledge:/);
...
async function assertOpenSearchAdapter() {
  ...
  const userDataPath = await fs.mkdtemp(path.join(os.tmpdir(), "splitall-external-kb-os-"));
  const mount = await createExternalKnowledgeBaseMount({
    userDataPath,
    runtimeOptions: {
      externalKnowledgeBase: {
