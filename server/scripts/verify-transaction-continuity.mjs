...
import { buildTransactionContinuityModel } from "../application/transaction-continuity-model.mjs";
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "splitall-continuity-"));
  ...
  assert.equal(contractPayload.schemaVersion, "splitall.transaction-knowledge.v2");
  assert.equal(contractPayload.overview.schemaVersion, "splitall.transaction-overview.v1");
  ...
  assert.ok(contractDocxXml.includes("splitall.transaction-overview.v1"), "DOCX should include machine-readable YAML overview");
