...
const host = String(args.host || process.env.SPLITALL_SERVER_HOST || "127.0.0.1").trim();
const port = normalizePort(args.port || process.env.SPLITALL_SERVER_PORT, 8787);
...
const runtimeOptions = {
  ...
  mountModules: {
    analysis: String(
      args["analysis-module"] || process.env.SPLITALL_SERVER_ANALYSIS_MODULE || ""
    ).trim(),
    ocr: String(args["ocr-module"] || process.env.SPLITALL_SERVER_OCR_MODULE || "").trim(),
    multimodalParser: String(
      args["multimodal-parser-module"] ||
        process.env.SPLITALL_SERVER_MULTIMODAL_PARSER_MODULE ||
        ""
    ).trim(),
    documentParser: String(
      args["document-parser-module"] || process.env.SPLITALL_SERVER_DOCUMENT_PARSER_MODULE || ""
    ).trim(),
    pdfProcessor: String(
      args["pdf-processor-module"] || process.env.SPLITALL_SERVER_PDF_PROCESSOR_MODULE || ""
    ).trim(),
    knowledgeBase: String(
      args["knowledge-base-module"] || process.env.SPLITALL_SERVER_KNOWLEDGE_BASE_MODULE || ""
    ).trim(),
    vectorStore: String(
      args["vector-store-module"] || process.env.SPLITALL_SERVER_VECTOR_STORE_MODULE || ""
    ).trim(),
    graphStore: String(
      args["graph-store-module"] || process.env.SPLITALL_SERVER_GRAPH_STORE_MODULE || ""
    ).trim()
  }
};
const discoveryOptions = {
  serverId: String(args["server-id"] || process.env.SPLITALL_SERVER_ID || "").trim(),
  serverLabel: String(args["server-label"] || process.env.SPLITALL_SERVER_LABEL || "").trim(),
  bootstrapBaseUrl: String(
    args["bootstrap-url"] || process.env.SPLITALL_BOOTSTRAP_URL || ""
  ).trim(),
  advertisedBaseUrl: String(
    args["advertised-base-url"] || process.env.SPLITALL_ADVERTISED_BASE_URL || ""
  ).trim(),
  activeServiceUrl: String(
    args["active-service-url"] || process.env.SPLITALL_ACTIVE_SERVICE_URL || ""
  ).trim(),
  forwardBaseUrl: String(
    args["forward-to-url"] || process.env.SPLITALL_FORWARD_TO_URL || ""
  ).trim(),
  mode: String(
    args["discovery-mode"] || process.env.SPLITALL_DISCOVERY_MODE || "active"
  ).trim(),
  configVersion: String(
    args["config-version"] || process.env.SPLITALL_DISCOVERY_CONFIG_VERSION || ""
  ).trim(),
  refreshIntervalSeconds:
    args["refresh-interval-seconds"] ||
    process.env.SPLITALL_DISCOVERY_REFRESH_INTERVAL_SECONDS ||
    "",
  checkInIntervalSeconds:
    args["check-in-interval-seconds"] ||
    process.env.SPLITALL_DISCOVERY_CHECK_IN_INTERVAL_SECONDS ||
    "",
  offlineAfterSeconds:
    args["offline-after-seconds"] ||
    process.env.SPLITALL_DISCOVERY_OFFLINE_AFTER_SECONDS ||
    ""
};
...
console.log(`SplitAll server is running at ${serverHandle.url}`);
