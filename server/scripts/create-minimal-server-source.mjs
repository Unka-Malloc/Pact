...
const DEFAULT_OUTPUT = path.join(REPO_ROOT, "splitall-v1");
const DEFAULT_DOCS = [
  "Architecture.md",
  "SERVER.md",
  "SERVER_WEB.md",
...
function usage() {
  return [
    "Usage:",
    "  node server/scripts/create-minimal-server-source.mjs [--output splitall-v1] [--force]",
    "",
    "Options:",
    "  --output PATH          Target source tree directory. Default: ./splitall-v1",
...
function packageJsonForTarget({ rootPackage, dependencies, edition }) {
  return {
    name: "splitall-v1-minimal-server",
    version: rootPackage.version || "0.1.0",
    private: true,
    type: "module",
    bin: {
      splitall: "server/scripts/splitall.mjs"
    },
    scripts: {
...
async function writeReadme({ targetPath, featureRuntime }) {
  await fs.writeFile(
    ...
    [
      "# splitall-v1",
      "",
      "Minimal SplitAll server source tree generated from FeatureManifest.",
      "",
      "## Feature Profile",
      "",
