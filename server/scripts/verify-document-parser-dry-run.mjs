...
const userDataPath = await fs.mkdtemp(path.join(os.tmpdir(), "splitall-document-parser-dry-run-"));
...
try {
  await installAuthenticatedFetch(splitAllServer);

  const beforeJobs = await fetchJson(`${splitAllServer.url}/api/jobs?limit=20`);
  ...
  const directParsed = await fetchJson(`${splitAllServer.url}/api/knowledge/document-parser/parse`, {
    method: "POST",
    headers: {
  ...
  const uploadSession = await fetchJson(`${splitAllServer.url}/api/upload-sessions`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
  ...
  await fetchJson(
    `${splitAllServer.url}/api/upload-sessions/${encodeURIComponent(
  ...
  const parsed = await fetchJson(`${splitAllServer.url}/api/knowledge/document-parser/parse`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
  ...
  const uploadSessionAfterParse = await fetchJsonResponse(
    `${splitAllServer.url}/api/upload-sessions/${encodeURIComponent(uploadSession.sessionId)}`
  );
  const afterJobs = await fetchJson(`${splitAllServer.url}/api/jobs?limit=20`);
