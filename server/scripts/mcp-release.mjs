...
async function createPortableBundle({ outputDir, packageJson }) {
  ...
  await writeExecutable(path.join(stagingRoot, "install.command"), [
    ...
    "set -e",
    "DIR=$(CDPATH= cd -- \"$(dirname -- \"$0\")\" && pwd)",
    "printf 'AgentStudio URL [http://127.0.0.1:8787]: '",
    "IFS= read -r url",
    "url=${url:-http://127.0.0.1:8787}",
    "\"$DIR/agentstudio-mcp\" register --url \"$url\"",
    "\"$DIR/agentstudio-mcp\" install --url \"$url\"",
  ...
  await writeExecutable(path.join(stagingRoot, "doctor.command"), [
    ...
    "IFS= read -r url",
    "url=${url:-http://127.0.0.1:8787}",
    "\"$DIR/agentstudio-mcp\" doctor --url \"$url\"",
    "printf '\\nDone. Press Enter to close.'",
    "IFS= read -r _",
    ""
  ].join("\n"));
  await fs.writeFile(path.join(stagingRoot, "README.txt"), [
    ...
    "  printf '%s\\n' '<issued-token>' | ./agentstudio-mcp install --url http://127.0.0.1:8787 --target codex --token-stdin",
    "",
    "macOS double-click flow:",
    "  Open install.command, enter the AgentStudio URL, choose one or more clients, then enter the issued token.",
    "",
    "Uninstall:",
...
async function createBootstrapInstaller({ outputDir, packageJson, portable }) {
  ...
  const content = [
    ...
    "DEFAULT_AGENTSTUDIO_URL=\"http://127.0.0.1:8787\"",
    "BASE_URL=\"${AGENTSTUDIO_MCP_RELEASE_BASE_URL:-https://github.com/${REPO}/releases/latest/download}\"",
    "AGENTSTUDIO_URL=\"${AGENTSTUDIO_MCP_BASE_URL:-$DEFAULT_AGENTSTUDIO_URL}\"",
    ...
    "echo \"Downloading AgentStudio MCP connector $VERSION for $platform...\"",
    ...
    "\"$target_dir/agentstudio-mcp\" register --url \"$AGENTSTUDIO_URL\" >/dev/null",
    "echo \"Opening AgentStudio MCP client selector...\"",
    "exec \"$target_dir/agentstudio-mcp\" install --url \"$AGENTSTUDIO_URL\" \"$@\"",
    ""
  ].join("\n");
...
function releaseManifest({ channel, packageJson, tarballName, tarballPath, checksum, sizeBytes, portable, bootstrap }) {
  return {
    ...
    portable: {
      ...
      installCommand: `./${portable.executable} register --url <agentstudio-url>`,
      clientInstallCommand: `./${portable.executable} install --url <agentstudio-url> --target <client> --token-stdin`,
      doubleClickEntry: process.platform === "win32" ? "" : "install.command"
    },
    install: {
      githubOneLineCommand: bootstrap.oneLineCommand,
      registryCommand: `npx ${packageJson.name}@latest register --url <agentstudio-url>`,
      tarballCommand: `npm exec --package ./build/release/mcp/${tarballName} -- agentstudio-mcp register --url <agentstudio-url>`,
      portableCommand: `./${portable.executable} register --url <agentstudio-url>`,
      interactiveInstallCommand: `npx ${packageJson.name}@latest install`,
      clientInstallCommand: `npx ${packageJson.name}@latest install --url <agentstudio-url> --target <client> --token-stdin`,
      uninstallCommand: `npx ${packageJson.name}@latest uninstall --target <client>`,
      doctorCommand: `npx ${packageJson.name}@latest doctor --url <agentstudio-url>`,
