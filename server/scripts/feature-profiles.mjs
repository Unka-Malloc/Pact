...
function runBusinessChecks({ featureRuntime, activeOperations, clientPackagingConfig }) {
  ...
  if (featureRuntime.edition === "community") {
    ...
    assertCondition(!toolIds.has("splitall.knowledge.evolution.runs.create"), "community tool catalog must not expose knowledge evolution tool", failures);
...
async function verifySourceLayout(plan) {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), `splitall-feature-source-${plan.featureRuntime.edition}-`));
  try {
