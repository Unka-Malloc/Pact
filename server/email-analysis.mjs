...
function parseMetadataAddressList(metadata = {}, exactKeys = [], fragmentKeys = []) {
  const raw = metadataValues(metadata, exactKeys, fragmentKeys).join(", ");
  return parseAddressList(raw);
}

function pickStructuredEmailBody(source, fallbackText = "") {
  const preferredEmbedded = (source.embeddedDocuments || [])
    .filter((entry) => {
  ...
  return normalizeWhitespace(preferredEmbedded || source.text || fallbackText);
}
...
function parseEmailFromMetadata(source, generatedAt, ruleset) {
  ...
  const rawFrom = buildRawParticipant(
    firstMetadataValue(
      metadata,
      ["message:from", "message:from-email", "from"],
      ["message:from"]
    )
  );
  const rawTo = parseMetadataAddressList(
    metadata,
    ["message:to", "message:to-email", "to"],
    ["message:to"]
  );
  const rawCc = parseMetadataAddressList(
    metadata,
    ["message:cc", "message:cc-email", "cc"],
    ["message:cc"]
  );
  const rawBcc = parseMetadataAddressList(
    metadata,
    ["message:bcc", "message:bcc-email", "bcc"],
    ["message:bcc"]
  );
...
function inferPrimaryDomain(rawMessages) {
  ...
  for (const message of rawMessages) {
    ...
    for (const participant of participants) {
      ...
      counts.set(participant.domain, (counts.get(participant.domain) || 0) + 1);
    }
  }
...
function parseSourceAsEmail(source, relatedChunks, generatedAt, ruleset) {
  ...
  const rawTo = metadataEnvelope?.rawTo || parseAddressList(headers.to);
  const rawCc = metadataEnvelope?.rawCc || parseAddressList(headers.cc);
  const rawBcc = metadataEnvelope?.rawBcc || parseAddressList(headers.bcc);
  const emailSignals =
  ...
  return {
    ...
    references: metadataEnvelope?.references || parseReferenceIds(headers.references)
  };
}
