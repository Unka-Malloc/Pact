import {
  ...
  normalizeRisk,
  riskRank
} from "../../application/MaintenanceAgent/config.mjs";
...
const READ_ONLY_POST_OPERATION_IDS = new Set([
  ...
  "agent_tools.knowledge_render_markdown"
]);
...
const REQUIRED_SCOPE_DECORATORS = new Map([
  ...
  ["tool_platform.get", ["runtime:admin"]],
  ["tool_platform.create_grant", ["runtime:admin"]],
  ["tool_platform.update_grant", ["runtime:admin"]],
  ["tool_platform.delete_grant", ["runtime:admin"]],
  ["tool_platform.rotate_grant", ["runtime:admin"]]
]);
...
const SAFETY_DECORATORS = new Map([
  ...
  ["tool_platform.create_grant", { risk: "repair_write" }],
  ["tool_platform.update_grant", { risk: "repair_write" }],
  ["tool_platform.delete_grant", { risk: "repair_write" }],
  ["tool_platform.rotate_grant", { risk: "repair_write" }],
  ["email_rules.set", { risk: "repair_write" }],
  ["expert_vocabulary.set", { risk: "repair_write" }],
  ["knowledge_packages.create", { risk: "repair_write" }],
  ["knowledge_packages.update", { risk: "repair_write" }],
  ["knowledge_packages.publish", { risk: "repair_write" }],
  ["knowledge_packages.rollback", { risk: "repair_write" }],
...
const CONCURRENCY_GROUP_DECORATORS = new Map([
  ...
  ["expert_vocabulary.set", "knowledge.packages"],
  ["knowledge_packages.create", "knowledge.packages"],
  ["knowledge_packages.update", "knowledge.packages"],
  ["knowledge_packages.publish", "knowledge.packages"],
  ["knowledge_packages.rollback", "knowledge.packages"],
...
function normalizeOperationContract(operation) {
  ...
  const externalAuth =
    ...
    String(operation.id || "").startsWith("agent_tools.");
  return {
    ...operation,
    public: publicAccess,
    externalAuth,
