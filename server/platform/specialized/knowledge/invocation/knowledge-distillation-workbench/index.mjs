...
export const KNOWLEDGE_DISTILLATION_WORKBENCH_PROTOCOL_VERSION =
  "splitall.knowledge-distillation-workbench.v1";
...
function markdownToHtml(markdown = "") {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>SplitAll Knowledge Distillation</title></head><body>${html.join("\n")}</body></html>`;
}
...
function buildRawConversionStage({ run, manifest, sources }) {
  ...
  return stageOutput("原始语料格式转换", markdown, {
    conversionPackage: {
      outputFormat: "docx",
      packageType: manifest.packageType || "splitall.normalized-documents",
    ...
    sections: [
      {
        children: [
          ...
          humanParagraph("SplitAll 知识蒸馏工作台阶段导出。"),
