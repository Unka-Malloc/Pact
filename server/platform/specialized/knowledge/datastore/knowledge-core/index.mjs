...
import { createEmbeddingRuntime as createProtocolEmbeddingRuntime } from "../../embedding/embedding-runtime/index.mjs";
import {
  SQLITE_VEC_PROVIDER_ID,
  createLocalVectorStore as createProtocolLocalVectorStore
} from "../../embedding/vector-store/LocalVectorStore/index.mjs";
import {
  LEARNING_PROTOCOL_VERSION,
  createLearningRuntime
} from "../../runtime/learning-runtime/index.mjs";
import { createKnowledgeTaxonomyRuntime } from "../../domain/knowledge-taxonomy/index.mjs";
import {
...
function tokenOverlapRatio(query = "", text = "") {
...
function localQueryHitsFromInput(input = {}) {
  const sources = [
    input.localQuery,
...
function localMirrorSourceLocator(hit = {}) {
...
function sourceLocatorDedupeKey(source = {}) {
...
function normalizeLocalMirrorHit({ hit = {}, query = "", settings = {}, referenceMs = Date.now(), index = 0 } = {}) {
...
function fusedServerSearchKey(item = {}) {
...
function fuseLocalMirrorWithKnowledgeItems({
  items = [],
  localHits = [],
  query = "",
  settings = {},
...
function createNoopDocumentOutlineRuntime() {
  return {
...
async function resolveDocumentOutlineRuntime({ enabled = true } = {}) {
  if (!enabled) {
...
function createKnowledgeStore({ db, rootPath, taxonomyRuntime = null, outlineRuntime = createNoopDocumentOutlineRuntime() }) {
  ...
  function hierarchyNodesByIds(ids = []) {
    ...
    `).all(...uniqueIds, ...uniqueIds).map((row) => hydrateHierarchyNode(row, []));
  }

  function compactHierarchyNodesForReasoning({ query = "", batchId = "", limit = 80 } = {}) {
    ...
    const rows = db.prepare(`
      ...
      ORDER BY level ASC, updated_at DESC
      LIMIT ?
    `).all(batchId, batchId, safeLimit);
    return rows.map((row) => hydrateHierarchyNode(row, queryTokens));
  }

  function compactTreeForReasoning({ query = "", batchId = "", limit = 80 } = {}) {
    const nodes = compactHierarchyNodesForReasoning({ query, batchId, limit });
  ...
  function buildHierarchyPlan({ query, queryTokens, batchId = "", settings, safeLimit, reasoningDecision = null }) {
    ...
    const reasoningNodes = hierarchyNodesByIds(reasoningNodeIds);
