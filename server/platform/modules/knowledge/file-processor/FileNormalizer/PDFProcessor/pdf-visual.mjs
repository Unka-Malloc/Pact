...
async function resolvePythonCommand(settings = {}) {
  const explicitPath =
    ...
    process.env.SPLITALL_PDF_VISUAL_PYTHON_PATH ||
    settings.ocrPythonPath?.trim() ||
    process.env.SPLITALL_OCR_PYTHON_PATH ||
    "";
...
export async function extractPdfVisualElements({
  ...
  try {
    ...
    if (error?.code === "ENOENT") {
      throw createPdfVisualError(
        "未找到可用的 Python 运行环境。请填写 SPLITALL_PDF_VISUAL_PYTHON_PATH，或创建 .venv-pdf 并安装 pymupdf/pdfplumber。",
        "PDF_VISUAL_UNAVAILABLE"
      );
    }
