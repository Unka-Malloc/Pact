...
import { buildSearchTerms } from "../domain/rules/index.mjs";
...
export function createBatchRepository({ db, userDataPath }) {
  ...
  const persistRawObjectStmt = db.prepare(`
    INSERT INTO raw_mail_objects (
      ...
      original_source_path, source_container_path, storage_rel_path, sha256, byte_size, media_type,
  ...
  const persistSourceStmt = db.prepare(`
    INSERT INTO source_files (
      ...
      source_created_at, source_updated_at, source_collected_at, media_type, extracted_text, created_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  return {
    ...
    getBatchArtifactPaths(batchId) {
      return {
        batchId,
        objectBatchPath: path.join(getRawMailObjectRoot(userDataPath), batchId)
      };
    }
  };
}
