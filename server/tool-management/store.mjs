...
function readBearerToken(request) {
  ...
  return String(request?.headers?.["x-splitall-tool-token"] || "").trim();
}
...
export function createToolManagementStore({ userDataPath }) {
  ...
  function migrateLegacyToolPlatform() {
    const count = db.prepare("SELECT count(*) AS count FROM tool_grants").get().count;
    if (count > 0) {
      return { migrated: false, reason: "grants_exist" };
    }
    const legacyPath = path.join(userDataPath, "tool-platform.json");
    ...
    try {
      ...
      return { migrated: false, reason: error instanceof Error ? error.message : "legacy_parse_failed" };
    }
    const grants = Array.isArray(legacy.grants) ? legacy.grants : [];
    for (const grant of grants) {
      const normalized = normalizeGrantInput({
        ...grant,
        toolsets: scopesToToolsets(grant.scopes),
        tokenHash: grant.tokenHash,
        tokenPrefix: grant.tokenPrefix
      });
      upsertGrant(normalized);
      appendGrantEvent(normalized.id, "legacy_migrated", { source: "tool-platform.json" });
    }
    return { migrated: grants.length > 0, count: grants.length };
  }
