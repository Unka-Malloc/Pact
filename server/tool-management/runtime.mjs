...
import { dispatchOperation, findHttpOperation } from "../interfaces/api/operation-dispatcher.mjs";
import { sendJson } from "../interfaces/http/http-utils.mjs";
import {
  getRuntimeLogger,
  summarizeError,
  summarizeForLog
} from "../observability/runtime-logger.mjs";
import { traceContextFromRequest } from "../observability/trace-context.mjs";
...
function copyCapturedHeaders(captured, response, extraHeaders = {}) {
...
export function createToolExecutionRuntime({
  ...
  async function executeHttpOperation({ request, response, requestBody, url, method } = {}) {
    logTool("info", "tool_management.compat_http.requested", {
    ...
    if (!match) {
      logTool("debug", "tool_management.compat_http.skipped", {
        requestId: request?.__splitallRequestId || "",
        method,
        route: url?.pathname || "",
        reason: "no_operation_match"
      });
      return false;
    }
    const tool = registry.getToolByOperationId(match.operation.id);
    if (!tool) {
      logTool("debug", "tool_management.compat_http.skipped", {
        requestId: request?.__splitallRequestId || "",
        method,
    ...
    for (const queryParam of match.operation.http?.query || []) {
      const value = url.searchParams.get(queryParam.name);
      if (value !== null && value !== "") {
        input[queryParam.name] = value;
      }
    }
    const result = await executeTool({
      ...
      request,
      context: {
        compatibilityRoute: url.pathname
      },
      directOperation: match.operation,
      directUrl: url,
      directRequestBody: requestBody,
      directParams: match.pathParams
    });
    if (result.captured) {
      const extraHeaders = {
        "X-SplitAll-Tool-Execution-Id": result.payload?.toolExecutionId || "",
        "X-SplitAll-Policy-Decision-Id": result.payload?.policy?.decisionId || ""
      };
      if (result.payload?.grant?.id) {
        extraHeaders["X-SplitAll-Tool-Grant-Id"] = result.payload.grant.id;
      }
      copyCapturedHeaders(result.captured, response, extraHeaders);
      response.writeHead(result.status, {
        ...result.captured.headers,
        ...extraHeaders
      });
      response.end(capturedBuffer(result.captured));
      return true;
    }
    sendJson(response, result.status || 500, result.payload);
    return true;
  }
