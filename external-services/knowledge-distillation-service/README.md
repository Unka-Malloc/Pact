...
- XLSX extraction preserves workbook sheet name/id/state/path, header row, row number, cell reference, style index, date number format, normalized ISO date, original date serial, formula, and `header=value` pairs so table evidence can be classified and grounded without losing cell context.
...
- `office-document-professional-adaptation.v1`: exposes a professional adapter matrix and per-document parsing/conversion profiles for PDF, Word, PowerPoint, Excel, Markdown, and OpenDocument, separating human-readable exports from agent-readable JSON/evidence packs while recording quality gates, including Office image references and Excel date serial normalization, and known loss boundaries.
...
- Unstructured-style element families for Markdown, markup, and structured-document headings, paragraphs, lists, links, images, table headers, table rows, code, citations, formulas, Word/PowerPoint/OpenDocument hyperlinks, PowerPoint placeholders, and spreadsheet hyperlinks.
