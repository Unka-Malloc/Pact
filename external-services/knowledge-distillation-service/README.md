...
- Office and OpenDocument: DOC/DOT/DOCX/DOCM/DOTX/DOTM, RTF, PPT/PPS/POT/PPTX/PPTM/PPSX/PPSM/POTX/POTM, XLS/XLSB/XLSX/XLSM/XLTX/XLTM, ODT/ODS/ODP with paragraph, heading, Word paragraph style and numbering refs, Word hyperlinks/images/comments/footnotes/endnotes, Word/PowerPoint/OpenDocument table row/cell metadata, OpenDocument hyperlinks, slide, PresentationML shape id/name, placeholder, and geometry refs, PowerPoint hyperlinks/images and speaker notes, workbook sheet name/id/state/path, sheet-row, cell-coordinate, SpreadsheetML merged-cell/date style/date serial/formula/hyperlink metadata, and table elements for OOXML/OpenDocument payloads.
- Ebooks: EPUB.
...
- PDF subtype routing emits `pdf.subtype-route`, `pdf.hyperlinks`, and `pdfProfile` so agents can distinguish text PDFs, scanned PDFs, font-mapping-risk PDFs, image-heavy PDFs, encrypted PDFs, empty/unknown PDFs, and PDF URI annotations without rescanning parser traces.
...
- DOCX and PPTX extraction preserves image relationship IDs, resolved package targets, alt text/title/description, and slide geometry where available, without loading binary media as text.
- XLSX extraction preserves workbook sheet name/id/state/path, header row, row number, cell reference, merged-cell range/master ref, style index, date number format, normalized ISO date, original date serial, formula, and `header=value` pairs so table evidence can be classified and grounded without losing cell context.
...
- `npm run server:verify:external-knowledge-distillation-container` builds the image, starts the service, checks `/v1/runtime/health`, and verifies OCR image, text PDF hyperlinks, image-only scanned PDF, legacy Office fallback, OpenDocument, EPUB, project ZIP/TAR/TGZ/7z packages, mounted file references, streaming JSONL document manifests, mounted archive packages, mounted PDF payloads, mounted DOCX/PPTX/XLSX/OpenDocument/EPUB payloads, signature-routed generic PDF/DOCX/PPTX/XLSX payloads, mounted DOC/PPT/XLS/RTF payloads, configuration files, markup files, diagram files, notebook files, source code files, diff/patch files, calendar files, MSG Tika extraction, MBOX mailbox splitting, and email attachment payloads become distillable corpus through `content.signature`, `ocr.image`, `ocr.page`, `tika.text`, `tika.text.file-ref`, `open-document.structured`, `open-document.tables`, `open-document.hyperlinks`, `office.word.styles`, `office.word.numbering`, `office.word.hyperlinks`, `office.word.images`, `office.presentation.placeholders`, `office.presentation.tables`, `office.presentation.hyperlinks`, `office.presentation.images`, `office.presentation.speaker-notes`, `ebook.epub`, `pdf.text.pdftotext`, `pdf.hyperlinks`, `structured-zip.file-ref.extract`, `table.workbook.sheets`, `table.sheet.headers`, `table.sheet.cells`, `table.sheet.merged-cells`, `table.sheet.date-styles`, `table.sheet.formulas`, `table.sheet.hyperlinks`, `table.time-index`, `config.key-value`, `markup.structure`, `diagram.structure`, `notebook.cells`, `code.structure`, `diff.unified`, `calendar.ics`, `archive.expand-route`, `archive.file-ref.expand`, `archive.entry-file-ref`, `archive.tar.container`, `archive.gzip.decompress`, `archive.7z.extract`, `input.manifest.jsonl`, `payload.file-ref`, `payload.stream-text`, `email.msg.tika`, `email.msg.tika.file-ref`, `email.mbox`, `email.mbox-route`, and `email.attachment-route`.
...
- `document-element-model.v1` and `element-aware-by-title-windowing.v1`: keep structured elements, heading paths, table/code/image/annotation isolation, element refs, Markdown links/images, basic PDF geometry and URI annotation links, Word paragraph style/list numbering refs, Word hyperlinks/images/annotations, Word/PowerPoint/OpenDocument table cells, OpenDocument hyperlinks, PowerPoint shape id/name/placeholders, PowerPoint hyperlinks/images and speaker notes, spreadsheet workbook sheet refs/cell coordinates/merged-cell ranges/date serials/formulas/hyperlinks, and PresentationML shape geometry on agent windows and graph text units.
...
- `office-document-professional-adaptation.v1`: exposes a professional adapter matrix and per-document parsing/conversion profiles for PDF, Word, PowerPoint, Excel, Markdown, and OpenDocument, separating human-readable exports from agent-readable JSON/evidence packs while recording quality gates, including Office image references, Excel merged-cell preservation, Excel date serial normalization, and known loss boundaries.
...
- Unstructured-style element families for Markdown, markup, and structured-document headings, paragraphs, lists, links, images, table headers, table rows, merged-cell ranges, code, citations, formulas, Word/PowerPoint/OpenDocument hyperlinks, PowerPoint placeholders, and spreadsheet hyperlinks.
