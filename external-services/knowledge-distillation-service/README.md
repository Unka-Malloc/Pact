...
- Office and OpenDocument: DOC/DOCX, RTF, PPT/PPTX, XLS/XLSX, ODT/ODS/ODP with paragraph, heading, Word/PowerPoint/OpenDocument table row/cell metadata, slide, PresentationML shape geometry, sheet-row, cell-coordinate, and table elements for OOXML/OpenDocument payloads.
- Ebooks: EPUB.
...
- Mounted structured ZIP file references for DOCX, PPTX, XLSX, ODT/ODS/ODP, and EPUB, with structural text extraction into chunked windows.
- XLSX extraction preserves sheet, header row, row number, cell reference, and `header=value` pairs so table evidence can be classified and grounded without losing cell context.
...
- `document-element-model.v1` and `element-aware-by-title-windowing.v1`: keep structured elements, heading paths, table/code isolation, element refs, basic PDF geometry, Word/PowerPoint/OpenDocument table cells, spreadsheet cell coordinates, and PresentationML shape geometry on agent windows and graph text units.
