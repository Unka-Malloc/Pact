...
const consolePhrasePairs: ConsolePhrasePair[] = [
  ...
  ["调试面板", "Debug Panel"],
  ["知识库召回", "Knowledge Recall"],
  ["智能体检索", "Agent Retrieval"],
  ["智能体", "Agents"],
  ["技能库", "Skill Hub"],
  ...
  ["知识录入发现重复来源、路径内容冲突或结构化版本冲突时，在工作台统一决策。", "When knowledge ingest finds duplicate sources, path/content conflicts, or structured-version conflicts, decisions are handled here."],
  ...
  ["添加目录", "Add Directory"],
  ["添加中", "Adding"],
  ["填写服务端可访问的本地目录。目录变化后会自动整理并更新知识库，也可以手动刷新。", "Enter local directories accessible to the server. Directory changes automatically trigger processing and knowledge updates, and can also be refreshed manually."],
  ["暂无目录", "No Directories"],
  ...
  ["模型库", "Model Library"],
  ["通用智能体", "General Agent"],
  ["新增需要使用的模型后填写授权，并可直接探测连通性。", "Add the models you need, fill in credentials, and probe connectivity directly."],
  ["智能体名称", "Agent Name"],
  ["模型 ID", "Model ID"],
  ...
  ["允许直接命令", "Allow Direct Commands"],
  ["命令 Timeout(ms)", "Command Timeout (ms)"],
  ["命令最大输出字节", "Command Max Output Bytes"],
  ["本地命令模板", "Local Command Templates"],
  ["激活 (active)", "Active"],
  ["调用框架", "Invocation Framework"],
  ["模型可输出 function call；服务端再按这里的 HTTP / 本地命令策略执行。命令使用 Node.js spawn，shell=false，跨平台。", "Models can emit function calls. The server executes them through the HTTP and local-command policies here. Commands use Node.js spawn with shell=false across platforms."],
  ["知识库召回", "Knowledge Recall"],
  ["只调试底层知识库召回，不调用大模型。适合检查融合策略、学习开关和证据可读性。", "Debug low-level knowledge recall without calling an LLM. Use this to inspect fusion strategy, learning flags, and evidence readability."],
  ["召回问题", "Recall Query"],
  ["知识库", "Knowledge Base"],
  ["召回模式", "Recall Mode"],
  ["仅关键词", "Keyword Only"],
  ...
  ["上传知识并选择入库目标，可同时进入全局知识空间、外部知识库、团队空间或用户私有空间。", "Upload knowledge and choose ingest targets. A single upload can enter the global space, external knowledge bases, team spaces, or user-private spaces."],
  ["入库目标", "Ingest Targets"],
  ...
  ["当前账号可见的内部与外部知识库。内部或外部是知识库状态，不作为配置选项。", "Internal and external knowledge bases visible to the current account. Internal or external is a knowledge-base state, not a configuration option."],
  ...
  ["只允许 secret:// 引用，不在管控台保存明文密钥。", "Only secret:// references are allowed. Plaintext secrets are not stored in the console."],
  ...
  ["报告序列、同义词和部门规则会进入邮件分析、分类引导和检索归纳。Toggle 会写入 email-rules.json 的 enabled 字段。", "Report series, synonyms, and department rules are used by mail analysis, classification guidance, and retrieval summarization. The toggle writes the enabled field in email-rules.json."],
  ["报告序列", "Report Series"],
  ["同义词", "Synonyms"],
  ["部门归属", "Department Mapping"],
  ...
  ["暂无审批", "No Approvals"],
  ["智能体权限", "Agent Permissions"],
  ["权限组是智能体管理页可选择的预设; 详细的权限层levels, 工具集和单工具例外只在这里维护.", "Permission groups are presets selectable from Agent Management. Detailed permission layers, toolsets, and per-tool exceptions are maintained here."],
  ["权限组是智能体管理页可选择的预设；详细的权限层 levels、工具集和单工具例外只在这里维护。", "Permission groups are presets selectable from Agent Management. Detailed permission layers, toolsets, and per-tool exceptions are maintained here."],
  ...
  ["无分组", "No Group"],
  ["未绑定用户", "No Bound User"],
  ["保存智能体权限组失败。", "Failed to save agent permission groups."],
  ...
  ["数据源能力", "Data Source Capabilities"],
  ["配置并管理所有的本地或外部数据来源。", "Configure and manage all local or external data sources."],
  ["外部客户端", "External Clients"],
  ["桌面客户端通过服务发现接入，服务端只提供任务、解析与工具能力。", "Desktop clients connect through service discovery. The server only provides jobs, parsing, and tool capabilities."],
  ...
  ["运维监控", "Ops Monitor"],
  ["统一查看服务端进程状态、守护巡检和报警队列。", "View server process status, guard inspections, and alert queues in one place."],
  ["客户端热力图", "Client Heatmap"],
  ["进程状态", "Process Status"],
  ["监控报警", "Monitoring Alerts"],
  ["报警报文配置 JSON", "Alert Payload Config JSON"],
  ["最近报警历史", "Recent Alert History"],
  ["生产健康", "Production Readiness"],
  ["日志记录", "Logs"],
  ["工具目录与微调", "Tool Catalog and Tuning"],
  ["策略与调用分析", "Policy and Invocation Analytics"],
  ["网关授权分配", "Gateway Grant Assignment"],
  ["后台报警", "Background Alert"],
  ["后台守护进程", "Background Supervisor"],
  ["后台守护进程离线", "Background Supervisor Offline"],
  ["后台进程守护器", "Background Process Supervisor"],
  ["导入解析 Worker", "Import Parser Worker"],
  ["目录同步 Worker", "Directory Sync Worker"],
  ["维护任务 Worker", "Maintenance Worker"],
  ...
  ["已接入搜索排序链路, 控制关键词, 向量, 图片, 图谱, 反馈和分层索引的融合权重.", "Connected to the search ranking chain. Controls fusion weights for keyword, vector, image, graph, feedback, and hierarchical indexes."],
  ["已接入搜索排序链路，控制关键词、向量、图片、图谱、反馈和分层索引的融合权重。", "Connected to the search ranking chain. Controls fusion weights for keyword, vector, image, graph, feedback, and hierarchical indexes."],
  ["BM25 权重", "BM25 Weight"],
  ["向量权重", "Vector Weight"],
  ["图片权重", "Image Weight"],
  ...
  ["刷新报警", "Refresh Alerts"],
  ["按协议层 clientUid 收敛工作空间, 上下文和使用热度, 低频连接会进入冷却状态.", "Aggregates workspaces, context, and usage heat by protocol-level clientUid. Low-frequency connections enter cooldown."],
  ["按协议层 clientUid 收敛工作空间、上下文和使用热度，低频连接会进入冷却状态。", "Aggregates workspaces, context, and usage heat by protocol-level clientUid. Low-frequency connections enter cooldown."],
  ...
  ["信息流最终报告需要一个可用智能体来融合原文检索、智能规划和附件结果。", "The final feed report needs an available agent to merge source retrieval, intelligent planning, and attachment results."],
  ["智能检索执行智能体", "Agent Retrieval Executor"],
  ...
  ["工作台审批流中的融合分析需要显式绑定一个可用智能体.", "Fusion analysis in Workbench approvals must explicitly bind an available agent."],
  ["工作台审批流中的融合分析需要显式绑定一个可用智能体。", "Fusion analysis in Workbench approvals must explicitly bind an available agent."],
  ["事务归类", "Transaction Classification"],
  ["邮件和文档进入图谱前的领域, 关键词和意图抽象.", "Domain, keyword, and intent abstraction before mail and documents enter the graph."],
  ["邮件和文档进入图谱前的领域、关键词和意图抽象。", "Domain, keyword, and intent abstraction before mail and documents enter the graph."],
  ["知识图谱增强", "Knowledge Graph Enhancement"],
  ["节点聚合, 关系解释和高频实体抽象.", "Node aggregation, relationship interpretation, and high-frequency entity abstraction."],
  ["节点聚合、关系解释和高频实体抽象。", "Node aggregation, relationship interpretation, and high-frequency entity abstraction."],
  ["时间线提炼", "Timeline Distillation"],
  ["围绕具体事务提炼阶段, 事件和关键节点.", "Distill stages, events, and key milestones around specific transactions."],
  ["围绕具体事务提炼阶段、事件和关键节点。", "Distill stages, events, and key milestones around specific transactions."],
  ["智能体工具调用", "Agent Tool Invocation"],
  ["智能体使用服务端工具前的意图理解和结果整理.", "Intent understanding and result formatting before agents use server-side tools."],
  ["智能体使用服务端工具前的意图理解和结果整理。", "Intent understanding and result formatting before agents use server-side tools."],
  ["所有", "All"],
  ["已解决", "Resolved"],
  ["已忽略", "Ignored"],
  ["全部", "All"],
  ["刷新授权", "Refresh Authorizations"],
  ["刷新冲突", "Refresh Conflicts"],
  ["添加本地目录", "Add Local Directory"],
  ["活跃服务", "Active Service"],
  ["模式", "Mode"],
  ["草稿", "Draft"],
  ["上下文编译器", "Context Compiler"],
  ["每次调用无状态模型前, 本地把记忆, 证据, 专家意见和工具状态编译成可审计 ContextPack.", "Before each stateless model call, local memory, evidence, expert guidance, and tool state are compiled into an auditable ContextPack."],
  ["每次调用无状态模型前，本地把记忆、证据、专家意见和工具状态编译成可审计 ContextPack。", "Before each stateless model call, local memory, evidence, expert guidance, and tool state are compiled into an auditable ContextPack."],
  ["导出记录", "Export Records"],
  ["窗口", "Window"],
  ["历史", "History"],
  ...
  ["已接入维护健康检查和重建流程, 控制重建批大小, 索引过期判断和图片证据质量门槛.", "Connected to maintenance health checks and rebuild workflows. Controls rebuild batch size, stale-index detection, and image evidence quality gates."],
  ["重建批大小", "Rebuild Batch Size"],
  ["过期索引小时数", "Stale Index Hours"],
  ["图片必须带 OCR 或说明", "Images Require OCR or Description"],
  ["已接入 embedding runtime, 用于选择文本, 图片和版本化重算索引的 provider.", "Connected to the embedding runtime for choosing text, image, and versioned reindex providers."],
  ["模型版本", "Model Version"],
  ...
  ["已接入维护健康检查和重建流程, 控制Rebuild Batch Size, 索引过期判断和图片证据质量门槛.", "Connected to maintenance health checks and rebuild workflows. Controls rebuild batch size, stale-index detection, and image evidence quality gates."],
  ["已接入维护健康检查和重建流程，控制Rebuild Batch Size、索引过期判断和图片证据质量门槛。", "Connected to maintenance health checks and rebuild workflows. Controls rebuild batch size, stale-index detection, and image evidence quality gates."],
  ...
  ["补齐恢复演练。", "Complete the recovery rehearsal."],
  ["刷新任务", "Refresh Jobs"],
  ["刷新队列监控", "Refresh Queue Monitor"],
  ["队列状态", "Queue Status"],
  ["这里收拢 queue-monitor, 导入解析, 知识蒸馏和智能巡检任务队列.", "This consolidates queue-monitor, import parsing, knowledge distillation, and inspection-agent task queues."],
  ["这里收拢 queue-monitor、导入解析、知识蒸馏和智能巡检任务队列。", "This consolidates queue-monitor, import parsing, knowledge distillation, and inspection-agent task queues."],
  ["监控项", "Monitor Items"],
  ["打开", "Open"],
  ["队列", "Queue"],
  ...
  ["当前筛选条件下没有匹配任务。", "No jobs match the current filters."],
  ["日志记录", "Logs"],
  ["汇总服务端上传, 知识库, 工作队列, 任务, 进程, 报警, 认证和工具调用日志.", "Aggregates server upload, knowledge base, work queue, job, process, alert, auth, and tool-invocation logs."],
  ["汇总服务端上传、知识库、工作队列、任务、进程、报警、认证和工具调用日志。", "Aggregates server upload, knowledge base, work queue, job, process, alert, auth, and tool-invocation logs."],
  ["显示", "Visible"],
  ["刷新日志", "Refresh Logs"],
  ...
  ["搜索 标签、ID、主机或系统…", "Search label, ID, host, or system..."],
  ["导入", "Import"],
  ["配置 未上报", "Config Not Reported"],
  ["未上报", "Not Reported"],
  ["未接入", "Not Connected"],
  ["暂无匹配客户端", "No Matching Clients"],
  ["请尝试更换搜索条件或检查网络连接.", "Try changing the search criteria or checking the network connection."],
  ["请尝试更换搜索条件或检查网络连接。", "Try changing the search criteria or checking the network connection."],
  ["迁移控制", "Migration Control"],
  ["已切换", "Switched"],
  ["迁移中", "Migrating"],
  ["客户端版本", "Client Version"],
  ["目标服务", "Target Service"],
  ["任务服务", "Job Service"],
  ["无运行任务", "No Running Job"],
  ["最近上报", "Last Reported"],
  ["发布中", "Publishing"],
  ["拉起迁移", "Start Migration"],
  ["暂无 pact-client 迁移流量", "No pact-client migration traffic"],
  ["引导地址", "Bootstrap URL"],
  ...
  ["冷却策略", "Cooling Policy"],
  ["冷却阈值", "Cooling Threshold"],
  ["保温客户端", "Retained Clients"],
  ["状态文件", "State File"],
  ["配置会保存到后台文件, 守护巡检下一轮自动读取", "Configuration is saved to the background file and read automatically on the next guard inspection."],
  ["配置会保存到后台文件，守护巡检下一轮自动读取", "Configuration is saved to the background file and read automatically on the next guard inspection."],
  ["提示", "Notice"],
...
const consolePatternPairs: ConsolePatternPair[] = [
  ...
  {
    zh: /^后台守护进程未运行[，,]\s*PID\s+(.+?)[。.]请检查 launchd 服务\s+(.+?)[。.]$/,
    en: (match) => `Background Supervisor is not running, PID ${match[1]}. Check launchd service ${match[2]}.`,
    enPattern: /^Background Supervisor is not running,\s*PID\s+(.+?)\. Check launchd service\s+(.+?)\.$/,
    zhBack: (match) => `后台守护进程未运行, PID ${match[1]}.请检查 launchd 服务 ${match[2]}.`,
  },
  {
    zh: /^活跃\s*·\s*(.+)$/,
  ...
  {
    zh: /^队列状态文件[:：]\s*(.+?)[;；]\s*事件日志[:：]\s*(.+)$/,
    en: (match) => `Queue state file: ${match[1]}; event log: ${match[2]}`,
    enPattern: /^Queue state file:\s*(.+?);\s*event log:\s*(.+)$/,
    zhBack: (match) => `队列状态文件: ${match[1]}; 事件日志: ${match[2]}`,
  },
  {
    zh: /^冷却策略[:：]\s*(.+?)\s*·\s*冷却阈值\s*(.+?)\s*分钟\s*·\s*保温客户端\s*(\d+)[;；]\s*状态文件[:：]\s*(.+)$/,
    en: (match) => `Cooling policy: ${match[1]} · cooling threshold ${match[2]} minutes · retained clients ${match[3]}; state file: ${match[4]}`,
    enPattern: /^Cooling policy:\s*(.+?)\s*·\s*cooling threshold\s*(.+?)\s*minutes\s*·\s*retained clients\s*(\d+);\s*state file:\s*(.+)$/,
    zhBack: (match) => `冷却策略: ${match[1]} · 冷却阈值 ${match[2]} 分钟 · 保温客户端 ${match[3]}; 状态文件: ${match[4]}`,
  },
  {
    zh: /^配置文件[:：]\s*(.+?)[;；]\s*sh 配置[:：]\s*(.+?)[;；]\s*状态文件[:：]\s*(.+)$/,
    en: (match) => `Config file: ${match[1]}; shell config: ${match[2]}; state file: ${match[3]}`,
    enPattern: /^Config file:\s*(.+?);\s*shell config:\s*(.+?);\s*state file:\s*(.+)$/,
    zhBack: (match) => `配置文件: ${match[1]}; sh 配置: ${match[2]}; 状态文件: ${match[3]}`,
  },
  {
    zh: /^(.+?)\s+已恢复$/,
